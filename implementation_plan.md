...
4. **Re-enable Supabase production deployment — BLOCKED on migration-version reconciliation.** The Supabase GitHub integration's **Deploy to production** option is **temporarily DISABLED** (turned off by Chris, 2026-08-25). **Automatic preview branching remains ENABLED** — each PR still gets its ephemeral preview project. It was disabled because **seven legacy repository migrations carry versions that do not match the versions recorded in production's `supabase_migrations.schema_migrations`**, so a production deployment run would treat them as unapplied and attempt to apply them — including a full baseline schema replay against the live database:
   `20260806000000_baseline_production_schema` · `20260807165600_campaign_leads_membership_uniqueness_and_attachment_core` · `20260807165610_import_campaign_creation_and_retry` · `20260807165620_dialer_session_campaign_access` · `20260812000000_client_policy_sold_draft_payment_fields` · `20260819000000_notifications_idempotency_recipients_security` · `20260820213208_get_dialer_campaign_presence_rpc`
   **These seven must be reconciled to their production-recorded versions before Deploy to production is re-enabled.** (The four inbound migrations were already aligned this way, which is why they are safe.) Until then, production schema and Edge Function changes are applied deliberately via the Supabase MCP `apply_migration` / `deploy_edge_function` path, never by a git merge.

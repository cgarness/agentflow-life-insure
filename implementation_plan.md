...
> Twilio reconciliation, the frontend release and TwiML activation all remain blocked until Phase 1
> is reviewed, separately approved for remote application, applied, and verified.
...
- **Sequencing (R20):** Phase 1 must be approved, authored, reviewed, applied, and verified **before the new `twilio-voice-inbound` TwiML that activates authoritative callback claiming is deployed**. M1/M2 and the inert callback endpoint may be prepared and deployed first; rollout stops before TwiML activation unless Phase 1 is live; the staging matrix (§13) runs only after Phase 1 is applied. No RLS approval is granted by the current review; no policy file exists yet.
...
- **RLS Phases 1 and 2** (§9) — not authored here; Phase 1 is a mandatory gate that **precedes activation of the new inbound TwiML** (R15/R20) and requires `#APPROVE_RLS_CHANGE`; Phase 2 is later privacy narrowing with its own inventory and tests.

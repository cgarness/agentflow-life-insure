...
## 9. RLS — diagnosis; Phase 1 is a MANDATORY gate that PRECEDES TwiML activation (R15/R20); Phase 2 is later privacy narrowing (R11). Nothing is authored in this build; `#APPROVE_RLS_CHANGE` still required.
...
4. **⛔ RLS Phase 1 gate (R15/R20) — BEFORE activation:** Chris issues `#APPROVE_RLS_CHANGE`; the Phase 1 command-split migration is then separately authored, reviewed, applied, and verified (including its §9 authorization matrix). **Rollout stops here without it — the new inbound TwiML is NOT deployed**, because activating authoritative callback claiming while any authenticated user can still directly preempt the winner would create a live bypass window.

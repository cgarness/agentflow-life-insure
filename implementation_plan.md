...
`AuthContext.checkProfileSetupNeeded` (`src/contexts/AuthContext.tsx:262-291`) early-returns `false` while the canonical wizard is still owed, then treats a **blank `phone` or `resident_state`** as "setup needed":
...
`markProfileSetupSeen` (`:293-301`) exists only to write that `agentflow-profile-setup-<uid>` key. **Repository-wide search confirms zero consumers** of `ProfileSetupModal`, `checkProfileSetupNeeded`, `markProfileSetupSeen` or `agentflow-profile-setup` outside `App.tsx`, `AuthContext.tsx` and the modal file (the only other hits are two historical `WORK_LOG.md` narrative lines, which stay). `needsAppOnboardingWizard` is imported into `AuthContext.tsx` (`:4`) **solely** for `checkProfileSetupNeeded`, so that import goes dead too.

**Root cause:** a legacy pre-wizard "profile completeness" nag survived the `/onboarding` redesign and now runs *downstream* of the canonical gate, treating optional CRM profile fields as an onboarding blocker.
...
SELECT upline_id INTO current_upline FROM public.profiles WHERE id = current_id;   -- committed row
...
useEffect(() => {
  ...
  usersApi.getAgentScopeIds({ viewerId: user.id, organizationId })
    ...
    .catch(e => {
      ...
      if (!cancelled) setAgentScopeIds([]);       // fail closed — never an org-wide fallback
    });
...
4. **Re-enable Supabase production deployment — BLOCKED on the baseline history consolidation.** The Supabase GitHub integration's **Deploy to production** option is **temporarily DISABLED** (turned off by Chris, 2026-08-25). **Automatic preview branching remains ENABLED** — each PR still gets its ephemeral preview project. It was disabled because **seven legacy repository migrations carried versions that did not match the versions recorded in production's `supabase_migrations.schema_migrations`**, so a production deployment run would treat them as unapplied and attempt to apply them against the live database. **Six of the seven are now reconciled** by repository-only `git mv` rename (2026-08-25, branch `claude/supabase-migration-history-reconciliation-g4kz4u`) — each proven 100% similarity with its Git blob and bytes unchanged, and each matching its production-recorded statement byte-for-byte:
   ...
   **`20260806000000_baseline_production_schema` remains the SOLE pending migration.** It has no production-recorded counterpart of any kind — no row at `20260806000000`, no row matching `%baseline%`, and nothing recorded between `20260805090000` and `20260811200920`. It therefore **cannot be reconciled by renaming**: the only defined procedure is the separately gated **baseline history consolidation** (`supabase/rollback/20260806_baseline_history_reconciliation_runbook.md` §S1 — revert **only** the 262 pre-baseline versions, then mark `20260806000000` applied, metadata-only via `supabase migration repair`), which remains **NOT PERFORMED and BLOCKED on Chris's explicit approval**. **S1's correct end state is 11 history rows — the baseline plus the ten already-applied post-baseline versions (`20260811200920`, `20260811201250`, `20260811201401`, `20260812042319`, `20260819163413`, `20260820233402`, `20260823203257`, `20260823222528`, `20260823222805`, `20260823222926`) — NOT one row.** Those ten are never reverted, never re-applied, and never named by any repair command; `npm run verify:s1-plan` statically enforces this. **Deploy to production must stay OFF until that consolidation completes** — no rename round can clear it, and the baseline additionally sorts *before* the lowest matched remote version, which the CLI refuses as an out-of-order pending migration. Until then, production schema and Edge Function changes are applied deliberately via the Supabase MCP `apply_migration` / `deploy_edge_function` path, never by a git merge.

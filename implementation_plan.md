...
**Status (updated 2026-08-10):** **AS-BUILT, committed and pushed on branch `bugfix/import-campaign-attachment` (draft PR #352)**, originally cut from `origin/main` = `4798ce4…` and now integrated with the merged migration-baseline repair (`main` = `1dbf295…`, PR #353) — a from-scratch replay is no longer blocked. **PR #352 is NOT merged; no PR #352 migration is applied to production; no Edge Function deployed; no production data mutated; neither production repair executed; S1/S2/S3 remain gated.** (The original 2026-08-07 status text recorded the pre-commit state; superseded by §30–§31.)
**Date:** 2026-08-07
...
## 31. PR #352 integration + official verification pass (approved by Chris 2026-08-10; executed in this integration pass)

Recorded before implementation per protocol. Scope: (a) merge current `origin/main` (`1dbf295`,
the merged PR #353 baseline repair) into `bugfix/import-campaign-attachment` as ONE additive merge
commit — merge-tree confirmed exactly two textual conflicts (`WORK_LOG.md`, `implementation_plan.md`)
plus a semantic invariant-number collision in the auto-merged `AGENT_RULES.md` (main owns #25 =
migration-baseline; this branch's import-campaign invariant renumbers #25→#26 and custom-field
identity #26→#27; the single historical "#25" reference in §30's table stays as written — accurate
when recorded — with the renumbering noted here). WORK_LOG keeps both histories (main's 2026-08-09
baseline entries above this branch's 2026-08-08/07 entries) + a newest-first integration entry
after the work; implementation_plan preserves this branch's plan and locked decisions AND the
baseline repair's durable operational sections, dropping only stale "missing baseline still blocks
replay" current-state wording. Never edited: the baseline migration, migrations_archive/**, the
generator/fingerprint scripts, M1–M3 bodies (absent a Chris-approved real defect). (b) Permanent
surgical fixes in `supabase/tests/import_campaign_attachment.sql` ONLY: T8 `_sys()` resets claims
with `set_config('request.jwt.claims','{}',true)` (NULL sets the GUC to '' which breaks
`''::json` inside claim readers — proven in the disposable replay; the unauthenticated assertion
is preserved); T33c gains a dedicated Admin-authorized fixture — a bounded same-org **Open Pool**
campaign created through the real `create_import_campaign` RPC as the Admin (T26c pattern),
requested instead of Team `…0002` (whose participants are AG1/AG2, so it was never Admin-dialable),
deleted in-transaction after the assertion; the T33c requirement is unchanged (stale unauthorized
Personal session must not satisfy an independently authorized request) and the Team fixture /
T7/T11/T26 coverage and production logic are untouched. (c) Fail-first evidence on a disposable
local stack BEFORE the fixes: unchanged suite fails at T8; after only the T8 fix it fails at T33c.
(d) Final official run from zero on the merged branch: baseline → M1 → M2 → M3, no seed, no
production connection; bootstrap via its documented local-only command (expect 324/0/4-empty);
T0–T45 under ON_ERROR_STOP, one transaction, final ROLLBACK, ALL ASSERTIONS PASSED; zero fixture
residue, zero cron jobs, zero net-queue rows. (e) App gates: tsc · focused PR #352 Vitest ·
full Vitest (host, UTC, America/Los_Angeles) · ESLint zero-new · build · literal
`git diff --check origin/main...HEAD` · full PR-diff + migration-ordering review.
`sql-tests.yml` stays manual-only (the six defective main suites remain separate follow-up work).
(f) Commits: the merge commit + ONE fixture/documentation commit; push only when all green; PR
description updated; PR stays DRAFT; both preview deployments monitored to a terminal state.
Authenticated preview verification is NOT claimed — it is the next separate gate. Absolute
exclusions honored: no S1/S2/S3, no migration repair, no db push, no production mutation, no
106-lead/32-row repairs, no Edge deploy, no RLS/TwilioContext/dial-path/telemetry changes, no
SQL CI activation, no merge/ready of PR #352.
...
## 7. Migration-baseline repair — approved plan (2026-08-09; branch `repair/migration-baseline`)

**Goal (Chris-approved, shortest safe path):** make a from-scratch replay possible so PR #352's SQL
gate can actually run, without redesigning production. Phase 0 (2026-08-08, GO) proved feasibility
in a disposable environment; this section records the approved implementation.

**Approved emergency hotfix executed first (2026-08-09, production, one statement, no
migration-history row):** `REVOKE ALL ON FUNCTION public.wipe_organization_operational_data(uuid)
FROM PUBLIC, anon, authenticated;` — the function was SECURITY DEFINER with no internal caller
check and anon/authenticated EXECUTE. Verified after: ACL `{postgres, service_role}`, body md5
unchanged (`4146d91c8780d82b381ed0b574b41a49`), no advisor finding, no data touched. Documented
inverse (never without separate approval, PUBLIC deliberately excluded) lives in the runbook.

**Architecture:** one guarded, production-derived baseline
`supabase/migrations/20260806000000_baseline_production_schema.sql` (regenerated post-hotfix dump +
Appendix A auth.users triggers + B storage buckets + C storage.objects policies + D generated ACL
sync); all 265 pre-baseline files preserved under `supabase/migrations_archive/pre_baseline/`
(README documents the five unreplayability classes and the ratified stale-trio dispositions —
phone_system_rls_harden and call_recordings_storage_update_policy archived as prod-equivalent /
would-weaken-isolation; leaderboard_rpc_tiebreak archived here and re-authored forward AFTER PR
#352 as its own reviewed migration). Active ordering: `20260805090000` (last applied) <
`20260806000000` (baseline) < `20260807165600` (PR #352 M1). No leaderboard file in this repair.

**Fingerprint contract:** authority is `scripts/fingerprint.sql` + `scripts/fingerprint_rollup.sql`
run identically against production (read-only) and the replay; the current category inventory and
numerator/denominator are recorded in WORK_LOG.md and the repair PR (superseded counts are never
embedded in standing docs). ACL appendices generated by `scripts/generate_acl_sync.sql`, never
hand-edited.
...
**Verification gates for this branch:** fresh `db reset --local --no-seed` exit 0 → fingerprint
per contract → ACL equality (incl. hardened wipe fn) → bootstrap applied via documented command and
row-counts verified → (main's six pre-existing SQL suites are known-defective — first-ever execution exposed latent
fixture defects that fail identically against production schema; fixing them and activating SQL CI
is separate follow-up work, NOT a gate here) → tsc/Vitest/build/lint/`git diff --check
origin/main...HEAD` (exit 0 via a narrowly scoped .gitattributes rule for the generated verbatim
baseline file only) →
zero prod URLs/tenant data/secrets in staged diff. PR #352's T0–T45 suite is NOT an official gate
here; disposable-environment integration evidence only (its T8/T33c fixture defects are fixed in a
reviewed PR #352 follow-up, never on this branch). SQL CI activates only for suites proven green.

**Release order (unchanged):** repair PR merges first → main merges into PR #352 → T8/T33c fixed
there → full replay + suite + app tests + authenticated preview → S1 metadata-only history
reconciliation (runbook) → S2 verification/advisors → S3 apply M1→M2→M3 → frontend last. The
106-lead and 32-row production repairs remain separately gated.

---

## 8. PR #353 bounded corrective pass (2026-08-09; Chris-approved scope, one additive commit)

Scope (nothing else): (1) strengthen the baseline guard to a multi-sentinel "initialized AgentFlow
database" check (9 core tables + private-schema sentinel + applied migration-history rows), proven
positive on a fresh stack and negative on a disposable partially-initialized schema, never against
production; (2) extend both fingerprint scripts with sequences (def/owner/ACL), default privileges,
replica identity, user-defined types (enum labels/domains), and function OWNER folded into the
function category — recompute and re-canonicalize the numerator/denominator, STOP on any
unexplained diff; (3) rewrite the S1 snapshot/inverse in the runbook: exact full-row export of all
262 schema_migrations rows (every column incl. complete statements arrays) to a permission-
restricted operator artifact OUTSIDE git, checksummed and count/version-verified, hard-stop if
unverifiable; distinguish version-presence restoration (migration repair) from exact row
restoration (transactional restore from the snapshot, separately approved); (4) narrowly-scoped
.gitattributes for ONLY the generated baseline file so the literal `git diff --check
origin/main...HEAD` exits 0; (5) remove the seven fabricated system_status rows from the bootstrap
(operational health claims, deprecated Telnyx/SendGrid branding — not reference data; production's
stale rows become a separate follow-up), new counts 324 area codes / 4 empty singletons / 0
system_status, and one exact docker-exec local-only apply command verified verbatim; (6) reconcile
every stale doc statement (11/13 removals; AGENT_RULES points at the fingerprint scripts as
authority WITHOUT embedded counts; implementation_plan no longer requires the six known-defective

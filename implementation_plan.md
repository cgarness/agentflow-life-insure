...
**Fingerprint contract (canonical wording):** 11/13 categories identical (10 byte-identical
non-empty + views empty-on-both-sides); 2/13 differ only by documented platform-internal objects
(local `pg_graphql` extension; production-internal `supabase_realtime_messages_publication::
realtime.messages`); 0 unexplained. Authority: `scripts/fingerprint_rollup.sql`; ACL appendix
generated by `scripts/generate_acl_sync.sql`, never hand-edited.
...
row-counts verified → main's six local SQL suites pass → tsc/Vitest/build/lint/`git diff --check` →

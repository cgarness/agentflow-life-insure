...
**Date:** 2026-08-20 · **Status:** **IMPLEMENTED LOCALLY; PRESERVED ON DRAFT PR #362; CORRECTIVE PASS APPLIED.** Approved for local development (Chris, 2026-08-20) with rulings **D1–D6 all approved as proposed** and **four mandatory amendments** (below); Chris then separately approved the preservation/review gate — implementation commit `2095445` was **committed, pushed, and opened as DRAFT PR #362** (all checks green incl. the isolated Supabase preview-branch migration replay), followed by the approved corrective pass (§15). **The production migration is APPLIED as version `20260820233402` (Chris-approved gate, 2026-08-20 — §16); still gated: PR ready/merge, frontend release, Edge deployments, further schema changes.** The earlier "commit/push/PR prohibited" and "migration not applied" wording in this header and the closing STOP line is superseded by those explicit approvals. This file supersedes the shipped Notifications Build 1 plan.
...
## 15. Corrective pass (Chris, 2026-08-20 — approved for draft PR #362 only; migration byte-untouched, blob `e4f981a1692842ae7f94b0fc64f036e1b54f2dab`)

Three findings, all fixed fail-first (10 red / 2 pins of 12 new tests against head `2095445`, then green):

1. **Partial presence response.** `headerPresence()` returned a partial sum (missing visible campaigns counted as zero). Now: **any** visible campaign absent from the returned map → `unavailable` (header hidden); the zero state renders only when **every** visible campaign has an authoritative zero row.
2. **Background presence failure.** React Query keeps last-good `data` through a failed background refetch, so stale live counts/identities could linger during an error. `useDialerCampaignPresence` now returns `{ presence, isError }` with `presence` **suppressed to undefined the moment the query is in an error state** — rows render "—", the header hides, leadership expanded presence renders "—". An ordinary in-flight refresh (no error) still keeps valid prior data visible.
3. **Expanded contact-state truthfulness.** The expanded Contact states section now mirrors the collapsed Contacts cell exactly: pending → "Loading counts…", errored-with-nothing-resolved → "—", authoritative empty → "No leads", resolved → chips. Also removed the unused `CampaignAvatarStack` import from `CampaignSelectionRowDetails.tsx`.

**Production preflight (read-only, 2026-08-20, post-PR):** `public.get_dialer_campaign_presence(uuid[])` **absent** from production and version `20260820213208` absent from `supabase_migrations.schema_migrations` (latest applied `20260819163413`); `public.can_dial_campaign(uuid)`, `private.campaign_actor()`, and `idx_dialer_sessions_org_status_heartbeat` all **present**. Production untouched.

**Expected post-apply advisor finding (documented in advance, do NOT expect a warning-free run):** the Supabase security advisor will flag this RPC as `authenticated_security_definer_function_executable` — the standard WARN carried by every authenticated-callable SECURITY DEFINER RPC in this project (`get_campaign_card_stats`, `get_queue_metrics`, `get_trusted_today_dialer_stats`, `get_org_leaderboard_stats`, …). **This warning is intentional for this RPC:** authenticated-executable is the entire point (Agents must receive aggregate counts their own-row RLS cannot provide), and the function is self-protected by the pinned `search_path = pg_catalog, pg_temp` with fully-qualified objects, explicit in-function authorization (`private.campaign_actor()` + canonical `can_dial_campaign` scope narrowing + DB-verified role gating of the identity payload), and an ACL that excludes PUBLIC and anon (EXECUTE only for `authenticated` + `service_role`). Any advisor finding beyond this expected one must be investigated, not waved through.

## 16. Production apply record (Chris-approved gate, executed 2026-08-20)

- **Applied:** ONE MCP `apply_migration` (`get_dialer_campaign_presence_rpc`) with the byte-identical file SQL — **recorded as production version `20260820233402`** (apply-time restamp; local file keeps `20260820213208`; blob `e4f981a1…` unchanged before and after). No other migration, no RLS/index/function change, no ad-hoc DDL.
- **Catalog verified:** single overload, SECURITY DEFINER, STABLE, exact `search_path=pg_catalog, pg_temp`, result shape `(campaign_id, active_agent_count, active_agents)`, ACL = authenticated + service_role only (no PUBLIC/anon), canonical `can_dial_campaign(c.id)` + `cardinality` + deterministic ordering present in the installed definition.
- **Live read-only checks (simulated claims, real profiles, zero writes):** Agent caller → rows ≡ canonical dialable set, counts ≡ direct 3-minute aggregate (authoritative zeros), identities always `[]`. Admin caller → exactly Open Pool + assigned Team, agent-owned Personal campaigns return NO row. Stale/ended/NULL-campaign session fingerprint byte-identical before/after (read-only proven). **BLOCKED honestly:** no live dialer session existed, so the leadership four-field identity payload and nonzero counts could not be exercised live (pinned by the SQL suite; visible on the next real session).
- **Advisor delta:** security 186 → 187 — exactly the §15-documented `authenticated_security_definer_function_executable` WARN on this RPC (intentional; not "fixed"); performance 369 → 369 identical.
- **Typegen (read-only):** `get_dialer_campaign_presence` appears with the expected Args/Returns; the repo types file was NOT overwritten this gate.

**STOP — the production migration is APPLIED (`20260820233402`) and verified. PR #362 ready/merge, frontend production release, Edge deployments, and any further application or migration change remain gated on Chris's separate explicit approvals. Repository hooks/automation are never treated as permission for a gated action.**

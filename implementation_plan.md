...
## 8. PR #347 review corrections (2026-08-05, Chris-approved, LOCAL ONLY — no commit/push/apply/merge/deploy in this pass)

**State when this pass started:** draft **PR #347** open (base `main`, head `bugfix/leaderboard-org-aggregate-rpc`), commit **`443f881`** pushed. The RPC is typed through a surgical `types.ts` `Functions` entry — **not** an `as any` cast. Neither `20260805090000` nor `20260614120000` is applied to production (re-verified read-only).

**Review findings and corrections (all approved, all local):**

- **A. SQL fixture validity.** The suite's direct `profiles` inserts violate the live **`profiles_id_fkey → auth.users(id) ON DELETE CASCADE`** (verified via `pg_constraint`), and the live **`on_auth_user_created → handle_new_user()`** trigger auto-creates a profiles row on every `auth.users` insert (deriving org/role from `raw_user_meta_data`; `'{}'` metadata ⇒ NULL org) — so a naive `ON CONFLICT DO NOTHING` profiles insert would silently keep the wrong auto-created row. Fix: adopt the established `p0_profile_authorization.sql` pattern — fixture-collision preflight; seed `auth.users` (`'{"provider":"email"}'` app metadata, `'{}'` user metadata); belt-and-braces `NOT EXISTS` profiles insert for trigger-less DBs; then **explicit `UPDATE public.profiles`** per fixture user forcing organization_id/role/status/names (session-role writes, `auth.uid()` NULL ⇒ the field-authorization guard's system path). Single `BEGIN…ROLLBACK` preserved. Never executed against production.
- **B. Loading lifecycle race.** In the hook, a **stale** fetch returns before `endFetch`, and a **silent** newest fetch never calls the visible settle — so a visible initial/filter fetch superseded by a silent poll leaves `initialLoading`/`filterRefreshing` stuck. Fix: `endFetch` (reachable only by the newest generation) now **always settles** both visible flags regardless of the completing fetch's silent option; stale fetches still never touch loading state. Fail-first tests: visible-initial + silent-supersede + late-stale-resolve ⇒ latest data retained, both flags false; plus the filter-refresh equivalent.
- **C. RPC scaling.** Replace the four per-profile correlated LATERAL scans with **organization/window-scoped grouped aggregates** (one grouped CTE each for calls, appointments via `COALESCE(created_by, user_id)`, wins/premium with the org-guarded clients join, recent wins) LEFT JOINed to the active roster. Return contract, metric canon, half-open bounds, auth checks, org isolation, grants, and the migration identifier **`20260805090000_get_org_leaderboard_stats_rpc.sql`** unchanged. Index review (read-only, live): keep only `idx_calls_org_created_at` — the grouped calls scan filters exactly `(organization_id, created_at)`; `wins` (`idx_wins_org`) and `appointments` (`idx_appointments_organization_id`) already have adequate paths at their cardinality; no blind indexes. EXPLAIN evidence: this machine has no psql/docker/local stack, so plan evidence comes from **read-only `EXPLAIN` (planning only, no execution)** of the grouped body vs the LATERAL body against the production schema, plus a results-parity SELECT; dev-branch EXPLAIN re-run happens with the T0–T11 execution step.
- **D. Dashboard widget regression coverage (approved scope addition: NEW `src/components/dashboard/__tests__/leaderboardWidget.test.tsx`).** Pins: org view calls `get_org_leaderboard_stats` with **zero** raw `clients`/`profiles` reads; `policies_sold` maps to Wins; initial error state + Retry; retained snapshot behind the stale note on a failed refresh; cancellation/stale-response discipline. Fail-first demonstrated by running the suite against the `origin/main` widget (temporary working-copy swap, restored immediately).
- **E. Governance.** New newest-first `WORK_LOG.md` entry (the 2026-08-05 checkpoint entry is append-only history and stays). **Sequencing rule: dev/local migration application + T0–T11 execution must PASS before any production application.** **Supabase advisor lint 0029 is an expected, explicitly reviewed warning** for this intentionally authenticated-executable SECURITY DEFINER aggregate (same class as every DEFINER RPC in this repo); the advisor acceptance criterion is **no unexplained new findings**, not "zero new findings". The draft PR description gets updated only when the corrections are pushed (a later, separately approved step) — GitHub is not modified during this local pass.

**Exact intended file list for this pass:** EDIT `supabase/tests/leaderboard_aggregate_rpc.sql` (A) · EDIT `src/hooks/useLeaderboardData.ts` (B) · EDIT `src/hooks/__tests__/useLeaderboardData.test.tsx` (B fail-first) · EDIT `supabase/migrations/20260805090000_get_org_leaderboard_stats_rpc.sql` (C, same identifier) · NEW `src/components/dashboard/__tests__/leaderboardWidget.test.tsx` (D) · EDIT `implementation_plan.md` (this section) · EDIT `WORK_LOG.md` (E). Nothing else — no RLS, no types.ts change, no GitHub, no migration application anywhere.

**Post-push syntax correction (2026-08-05, Chris-approved).** Review of `75edeb6` found one broken statement terminator in `supabase/tests/leaderboard_aggregate_rpc.sql`: the mechanical `ON CONFLICT` removal appended the calls-fixture INSERT's closing `;` to the tail of the last row's `-- org B` comment (line 153), leaving the statement unterminated. Fixed to `…'2026-07-01T10:00:00Z'); -- org B`. A sweep of the whole suite confirmed no other statement terminator is swallowed by a comment (every other `;`-in-comment hit is prose inside pure comment lines or mid-row comments after commas). Shipped as its own corrective commit; isolated dev-branch execution of the suite (Chris-approved, $0.01344/hour, deleted after) follows.

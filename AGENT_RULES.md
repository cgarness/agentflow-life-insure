...
- **Pass 2 (not yet built):** Personal numbers MUST be excluded from ALL automatic outbound selection — power-dialer rotation, campaign number rotation, AI/local presence, smart caller-ID, and fallback caller-ID. A Personal number may be **manually** selected **only by its owner** (`assigned_to = auth.uid()`); no other agent may see or use it. Caller-ID selection (`caller-id-selection.ts`, `local-presence.ts`, `TwilioContext`) is **unchanged in Pass 1** — do not gate on `assignment_type` until Pass 2 ships enforcement in the same deploy.
- **Number groups cannot override phone-number ownership/scope safety.** `number_groups` / `number_group_members` logic is unchanged in Pass 1.
- **Do not ship a live editable Agency/Personal Settings control before Pass 2 enforcement** (would risk the dialer burning a Personal number in shared local presence). Pass 1 Settings UI shows a **read-only** Agency/Personal badge only (`NumberManagementSection.tsx`). Pause/cool-off is **deferred to Pass 3** — no pause/cooldown columns, UI, or jobs in Pass 1/2.

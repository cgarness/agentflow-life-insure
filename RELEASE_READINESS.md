...
**Prepared:** 2026-09-13 (rev 6) · **Updated:** 2026-09-18 (rev 24) · **Status: M8 AND M9 APPLIED to production 2026-09-18 at recorded versions `20260918000614` and `20260918002859`. The full v2 schema (M4–M9) is now in place; everything else UNCHANGED — `recording-retention-purge` v30, `twilio-recording-status` v36, `twilio-voice-status` v42, `twilio-voice-inbound` v44, `inbound-call-claim` v38, every organization on legacy, v2 DISABLED. v30's byte-for-byte verification FAILURE still stands and is NOT retroactively passed (§2 Step 7). The corrected retention worker remains UNDEPLOYED and separately approved.** **M8 + M9 DO NOT BETWEEN THEM FIX THE STARVATION DEFECT** — deployed v30 still calls M7's `voicemails_cleanup_batch`; the new selectors exist but nothing calls them. M9's first-listen guard, by contrast, IS live and enforcing now. **Both voicemail defects remain GATES ON v2 ACTIVATION** until the corrected worker ships (§2 Steps 8–10 and the M8/M9 apply records below).
...
### Step 6b — `twilio-recording-status` — **CORRECTIVE FORWARD DEPLOYMENT COMPLETE AND FULLY VERIFIED AT v36 (2026-09-16)**

> **Production now runs v36.** Two narrow corrections to its **voicemail branch only** were authorised and are now deployed. **(2026-09-16, durable ownership recovery)** Establishing the owner in memory was not enough: a stored or purged row holding `provider_account_sid = NULL` had account B established from a signed callback, was DELETEd against B, took a provider 503 — and B was never written, so the next purge run saw NULL and reported `unresolved_ownership` forever. An authenticated callback that establishes a previously missing owner now **persists and verifies** it before anything contacts the provider, via a **guarded UPDATE of the owner column alone** bound to the exact voicemail, RecordingSid, call and organization with `provider_account_sid IS NULL` as its guard. `upsert_voicemail_from_recording` is deliberately not reused — its status expression would turn a **purged** row back into `stored` and resurrect purged media. Concurrent outcomes are classified: a matching owner is acceptable, a different established owner is a conflict that is never overwritten, and an unavailable or inconclusive result preserves the source and answers 503. The upsert's **returned** owner is authoritative and is checked, because M7 coalesces an existing value over the incoming one. **(Earlier the same day)** the branch defaulted the recording's owning account to the platform credential (`params["AccountSid"] ?? creds.accountSid`), used that for the DELETE, and persisted it through `p_account_sid` as authoritative. Ownership is now **established** — from the signature-validated callback, or from an already-stored matching owner on a retry — with an explicit `ownership_conflict` when the two disagree and an `ownership_unresolved` 503 (source preserved, nothing guessed, nothing persisted) when neither can establish it. **The ordinary conversation-recording branch is byte-identical to deployed v35** from `Deno.serve` onward.
>
> ```
> c5882571f7aee7f39c51d12b6fcf504db0d3a88f45e648c81fa4edc600f0bda6  functions/twilio-recording-status/idempotency.ts  20,552 bytes
> 93980e94e9eb854f71a6105a7a916645e0068eb8295284e33f66f65537a92659  functions/twilio-recording-status/index.ts        33,983 bytes
> ```
> **manifest `0893d95c57bce335589524bb2e6c57c5f5e30ed07dbaefb3b015b717165ae975`**, entrypoint `functions/twilio-recording-status/index.ts`, `verify_jwt = false`, no import map. **Deployed as v36 from commit `c79dff6cad63c99bf811308a6904b2f5634dec1e`.**

**Deployed and verified.** One `deploy_edge_function` call answered **version 36**, ACTIVE, `verify_jwt = false`, `import_map = false`, `ezbr_sha256 a75e7c80c877c3efdbf7bf9f08e6daf1079c0f94c8d47a1fea5d73473589fdca` (the v35 baseline read `640aadef…`). **The response was not treated as verification.** An independent `get_edge_function` readback returned both files, and their bytes were hashed directly from the returned payload:

```
readback  functions/twilio-recording-status/idempotency.ts  20,552 bytes  c5882571f7aee7f39c51d12b6fcf504db0d3a88f45e648c81fa4edc600f0bda6
readback  functions/twilio-recording-status/index.ts        33,983 bytes  93980e94e9eb854f71a6105a7a916645e0068eb8295284e33f66f65537a92659
```

Both are **byte-identical** to the repository at `c79dff6` and to the submitted payload. The manifest re-derived from the deployed bytes over the **approved submitted paths** is `0893d95c…` — equal to the approved manifest. The readback again lists names **without** the `functions/` prefix, so the two manifests are kept deliberately distinct: over the stripped names the same bytes hash to `104e85f49fd5733570154c7e91721d70cef4eff8ecbc91f7d9f7d7952f9a97f3`, and only the canonical form is compared against the approval.

**The pre-submission check earned its keep again.** Materialising the payload and comparing it to the repository *before* submitting caught **two dropped blank lines** (source lines 170 and 368) in the first materialisation. They were restored and the payload re-verified to `93980e94…` before any deployment call was made. This is the same class of defect — invisible whitespace — that produced the v41 failure, caught this time **before** it reached production.

**Prerequisites confirmed read-only, before deploying.** M4–M7 are applied at their recorded versions (`20260914000530`, `20260915025931`, `20260915035141`, `20260915053646`) and are still the four newest migrations. The `voicemails` bucket is **private**, `26 214 400` bytes, `audio/mpeg`. `voicemails.recording_sid` carries **`UNIQUE (recording_sid)`** plus a `^RE[0-9a-fA-F]{32}$` CHECK, so the callback's `.maybeSingle()` readback is exact. `service_role` holds **table-level and column-level UPDATE on `voicemails.provider_account_sid`** and is `BYPASSRLS`; there are **no triggers** on `voicemails` that could touch other columns behind the guarded write. All four RPCs the branch calls are present with the exact expected signatures, `SECURITY DEFINER`, `service_role` EXECUTE — **none was invoked as a health check**. The three `upsert_voicemail_from_recording` body facts the handler depends on were confirmed **in the live database**: it returns the whole row, it resolves the owner as `coalesce(v.provider_account_sid, EXCLUDED.provider_account_sid)` (**existing wins**, which is why the returned value is checked rather than assumed), and its status expression preserves `stored` (the resurrection hazard the guarded single-column write exists to avoid).

**Unchanged after the deployment, checked explicitly.** `twilio-voice-status` **42**, `recording-retention-purge` **29**, `twilio-voice-inbound` **44**, `inbound-call-claim` **38** — all ACTIVE, all `verify_jwt = false`, all carrying their prior bundle hashes. Routing is still **legacy**: the single `inbound_routing_settings` row reads `routing_engine = 'legacy'`, `voicemail_enabled = false`, and the second organization still has no row, which reads as legacy. **Zero** organizations are non-legacy. `voicemails` **0 rows**, `voicemails` bucket **0 objects**, `inbound_route_attempts` **0 rows**, `calls.routing_engine` NULL for all **1 869** rows, `calls.recording_source_sid` non-null on **14** rows before and after, **0** recording sentinel rows.

**Runtime behaviour of v36 is UNPROVEN, and that is separate from the package verification above, which is complete.** `twilio-recording-status` (`6eb57066…`) appears **nowhere** in `function_edge_logs` or `function_logs` across the full 24-hour window — not before the deployment and not after. **No natural callback has reached v36**, and none was generated: no live call, no synthetic callback, no recording upload, no manual cleanup request, no manual data edit. Live voicemail storage, durable ownership recovery, source deletion and notification convergence remain unverified until controlled testing. The blast radius stays nil for the same structural reason as v35: the new code is reached only when the callback URL carries `source=voicemail`, whose only producer is `twilio-voice-inbound`'s v2 path — still **v44**, undeployed, and gated by `routing_engine` regardless.
...
### M9 — **APPLIED TO PRODUCTION 2026-09-18, recorded version `20260918002859`**

> **READ THIS FIRST: one of my 35 post-apply checks FAILED, and the failure was in MY CHECK, not in the migration.** Per the approval I stopped writes and reconciled read-only before doing anything else. Full account below under *The failed check*. The migration itself applied byte-for-byte as approved and every security property it promises is verified.
>
> **What was authorized and done.** One `apply_migration` call against `jncvvsvckxhqgqvkppmj`, carrying the approved file at branch head `380cfd7901ffedd7433c9885187b74bff2b166d6`, submitted under the name `voicemail_first_listen_guard`. Nothing else: no Edge deployment, no purge invocation, no application-data change, no merge, no frontend release, no v2 activation, and no change to existing RLS policies, grants, retention periods, cron or secrets.
>
> **The recorded version is again not the authored filename.** Authored `20260918010000_…`; `apply_migration` stamps the version at apply time, so production recorded **`20260918002859`** — which still sorts after M8's `20260918000614`, so replay order is correct. The repository is reconciled to the recorded version; the approved content is unchanged and still hashes to `0442669…`.
>
> **Source fidelity, verified on both sides.** Before submitting, the payload was transcribed and compared byte for byte against the approved file: sha256 `0442669072c44a8ee8c3b3ceeadeff5fd25264a446e82baa9164aa2dca7f6583`, 7,233 bytes, 7,209 characters, with all 8 U+2014 and **4 U+2192** characters intact (M9 carries an arrow class M8 did not). After applying, the stored SQL was read back by the exact submitted name: **one** history row, one statement, 7,209 chars / 7,233 bytes, sha256 `0442669…` again. **Trailing-newline normalization was measured, not assumed:** the stored text already ends with LF and the hash taken *without* appending another newline is the matching one, so nothing was normalized.
>
> **THE FAILED CHECK, in full.** Check 16 read *"no grantee outside the OWNER"* and expected 0; production returned **1**. Read-only diagnosis established the cause immediately and unambiguously: the hosted project carries `ALTER DEFAULT PRIVILEGES` on functions in `public` granting EXECUTE to `postgres`, `anon`, `authenticated` **and `service_role`** — two rules, one owned by `postgres` and one by `supabase_admin`. Every new `public` function is therefore **born with all four grants**. M9 revokes PUBLIC, `anon` and `authenticated` — exactly the revocations this approval names — and deliberately does **not** revoke `service_role`, so that one entry survives. The resulting ACL, `postgres=X/postgres service_role=X/postgres`, is **identical to every sibling**: M7's `voicemails_cleanup_batch` and `voicemails_expired_batch` and M8's two selectors all carry exactly that shape.
>
> The disposable local stack has no such default privileges, so there the function was born owner-only and my wrong expectation went unnoticed. **The defect was in how I derived the expectation, not in the applied migration.** No production write was made to "fix" it; nothing was retried, repaired, rolled back or re-applied.
>
> The correction is the same allowance M8's verifier already used, and it is **not a weakening**. The security property lives in checks 13/14/15 (PUBLIC, `anon` and `authenticated` hold no EXECUTE) which are untouched, and three checks were **added** to make the ACL stricter than before: the grantee count must be exactly 2, the sorted grantee names must be exactly `postgres,service_role`, and every granted privilege must be EXECUTE. A negative control confirms the corrected check 16 still fires when `anon` or `authenticated` is granted EXECUTE, and the new checks fire if `service_role` is additionally revoked. `service_role` EXECUTE on a **trigger** function is inert regardless: trigger functions are invoked by the trigger machinery, never through EXECUTE (the migration's own comment says so), and `service_role` is precisely the role the guard exempts.
>
> The local stack was then rebuilt **with production's default privileges mirrored**, where it reproduces the exact production ACL and the corrected verifier passes. That is now the faithful preflight basis.
>
> **Post-apply assertions: 38 of 38 PASS** after the correction (35 original with check 16 corrected, plus checks 36–38), all from catalog reads. **No voicemail was seeded and no application RPC was invoked to "test" the trigger** — behaviour is proven by the local suite, identity by the catalog. They cover: exactly one `voicemails_enforce_first_listen()` in `public` and in no other schema, returning `trigger`, `plpgsql`, no arguments, **`SECURITY INVOKER`** (`prosecdef` false), pinned `search_path = pg_catalog, pg_temp`, and the approved body digest `18e17bd304a722cd176749a0a1e08b37`; the exemption testing **`current_user`** membership of `service_role`, guarded by `to_regrole`, and **not** keying on `session_user`; the ACL facts above; exactly one enabled (`tgenabled = 'O'`) row-level `BEFORE UPDATE OF listened_at` trigger on `public.voicemails` with `tgtype = 19` (ROW 1 + BEFORE 2 + UPDATE 16), bound to that function, naming exactly one column and that column being `listened_at`, definition digest `ce98ae4446da78332818ba360c5b201f`; both approved comments; M8's two selectors and two indexes and M7's `voicemails_cleanup_batch` unchanged; `voicemails_expired_batch` still present; `authenticated` and `anon` **not** members of `service_role`, so the exemption cannot reach a client role; and zero voicemail rows.
>
> **Sixteen of sixteen negative controls** detected their perturbation on the faithful local stack: EXECUTE granted to `authenticated`, to `anon`, to PUBLIC; `service_role` additionally revoked; `SECURITY DEFINER` instead of `INVOKER`; the exemption switched to `session_user`; `search_path` unpinned; the trigger disabled, dropped, made `AFTER`, made statement-level, or widened to all columns; a second trigger added; either comment removed; an M8 index dropped.
>
> **Behaviour (local suite, fresh):** `ALL INBOUND SQL SUITES GREEN (M1-M3 + v2 M4-M9, incl. both rollback proofs)`, including the L0 fail-first reproduction (with the guard disabled a forged timestamp sticks and expires fresh media) and L1–L8: first client listen stamped with the **server** clock and the client value discarded; an established timestamp immovable, unclearable and unresettable while the statement still reports success; no other column touched; row access still bounded by `can_access_voicemail` (recipient, eligible group member and org Admin allowed; unrelated same-org agent and cross-organization Admin denied); a forged clock can no longer expire fresh media; the guard confined to `UPDATE OF listened_at`; and the `service_role` maintenance exemption intact.
>
> **Unchanged-object comparison: 10 of 10 UNCHANGED**, captured before the apply and re-read after under a pinned `search_path` with `COLLATE "C"` ordering: policies `ba002d3e…` (2), RLS/force-RLS `true/false`, table grants `97e241f2…` (15), column grants `12e89431…` (226), **all** indexes including M8's `9eaf2ce6…` (10), the 9 other public voicemail functions `7ed5b03b…`, cron `45b0ed69…` over 9 jobs (each `command` hashed, so no credential is printed), routing `e87b5826…` with **zero organizations off legacy**, and zero voicemails / route attempts / v2 calls / storage objects. All five Edge packages identical by `ezbr_sha256`: purge v30 `6a4acf05…`, recording-status v36 `a75e7c80…`, voice-status v42 `d9bbe55c…`, voice-inbound v44 `4e0b95da…`, inbound-call-claim v38 `d6c5a82b…`.
>
> **Exactly what differs from the before-image**, and nothing else: the new function, the new trigger, the function's comment, and the `voicemails.listened_at` **column comment**, which was NULL before and now carries the approved text. That column comment is the single pre-existing object M9 touches and it is covered by the approval's "its comments"; its rollback sets it back to NULL.
>
> **Prerequisites confirmed read-only before submitting:** M4–M8 all recorded at their versions with M8 `20260918000614` / `voicemail_cleanup_actionable_selection` the highest; M9 unrecorded under either its version or its name; its function and trigger absent in every schema; no trigger at all on `voicemails`; M8's two selectors and two indexes present; zero voicemails, voicemail objects, route attempts and v2 calls; every organization on legacy; and **neither `authenticated` nor `anon` a member of `service_role`**, which is what keeps the exemption narrow.
>
> **Repository reconciliation.** `20260918010000_voicemail_first_listen_guard.sql` → **`20260918002859_…`**, rollback alongside. **Contents untouched, hash confirmed `0442669…`**; M4–M8 keep their recorded versions and M8's hash is re-confirmed `4324c679…`. References updated in `run_inbound_sql_tests.sh`, `run_cp13_rollback_test.sh`, `verify_inbound_generated_types.sh`, `verify_m9_post_apply.sql` and this document. The old authored filenames left in the two rollback header comments are untouched, as instructed. The WORK_LOG's M8 entry still names `20260918010000` for M9 because that is what was true when it was written; it is an append-only historical record and is deliberately not rewritten.
>
> **`verify_m8_post_apply.sql` was NOT re-run and was NOT weakened.** Its checks 39 and 40 assert M9 is absent, which was correct for the M8-only state it verified; it stands as historical evidence. `verify_m9_post_apply.sql` re-checks every M8 object it covered, so nothing is lost.
>
> **Re-run after the rename** (filenames feed them): `ALL INBOUND SQL SUITES GREEN` with the CP13 rollback proof's six negative controls detected, and generated-types green through M9 — including the assertion that the trigger function is **absent** from the generated `Functions`, which is what `postgres-meta` actually emits — with its four negative controls.
>
> **WHAT IS AND IS NOT NOW FIXED.** M9's guard **is live and enforcing**: `listened_at` is server-authoritative for every non-privileged caller from this moment. M8 + M9 still do **not** fix the starvation defect, because deployed `recording-retention-purge` v30 continues to call M7's unfiltered `voicemails_cleanup_batch` and nothing calls M8's selectors. The next release step is the already-reviewed corrected retention worker, whose deployment remains **separately approved**. v2 stays disabled, every organization stays on legacy, v30's failed byte-for-byte record is preserved, and Alexa's audible-ringing and routing incident remains unverified pending controlled live testing.
...
>
> **The defect, reproduced at `7d5e89e` through the real worker before anything was changed.** With an empty actionable batch and this response:
>
> ```json
> { "blocked_due": 0, "blocked_total": 0, "blocked_orgs": 0, "oldest_blocked_at": null, "scan_capped": true }
> ```
>
> `parseBlockedSummary` returned `{ok: true}` and `runVoicemailSourceCleanup` reported, in the same result: `status = completed`, `reason = no_work`, `queue_empty = true`, `blocked_scan_capped = true`, `blocked_summary_unavailable = false`. "The scan was truncated, so the real numbers are larger" and "there is no backlog and nothing is owed" cannot both be true, and the pass presented both at once. Identical output for the bare-object and singleton-array transport shapes.
>
> **Why it is impossible, derived from M8 rather than assumed.** `voicemails_cleanup_blocked_summary` clamps its scan limit with `least(greatest(coalesce(p_scan_limit, 5000), 1), 50000)`, so `n ≥ 1` for every input including `NULL`, `0` and negatives. It probes with `LIMIT n + 1`, sets `scan_capped := count(scanned) > n`, and returns `blocked_total := count(kept)` where `kept` is `scanned LIMIT n`. A true cap therefore means `count(scanned) > n ≥ 1`, which makes `count(kept) = n ≥ 1` and `blocked_total ≥ 1`. **A capped scan always retains at least one blocked row.** This is a malformed-response boundary, not evidence that production's SQL emits such a result.
>
> **The correction.** `parseBlockedSummary` now rejects `scan_capped = true` together with `blocked_total = 0`, returning the existing explicit unknown result with the reason `scan_capped is true but blocked_total is 0 (a capped scan retains at least one row)`. No new status vocabulary, no new reporting path: it joins the relationship checks added in pass 13b and flows through the same `blocked_summary_unavailable` handling. Everything that behaviour already guaranteed is unchanged and asserted:
>
> | Behavior | Preserved |
> | --- | --- |
> | An unknown summary suppresses clean completion and `queue_empty` | `status = partial`, `reason = blocked_summary_unavailable`, `queue_empty = false` |
> | Valid actionable rows are still processed | the rejected summary never gates the work — the row is deleted at the provider and reconciled |
> | A valid **zero** summary with `scan_capped = false` | still `completed / no_work / queue_empty = true` |
> | A valid **capped, positive** backlog | still believed and reported: `blocked_scan_capped = true`, counts intact, `partial / blocked_unresolved_ownership` |
>
> A rejected summary is also not believed in part: `blocked_scan_capped` stays `false`, so no capped claim survives alongside the unknown verdict.
>
> **Regression evidence.** The new tests were run first against the **unmodified** helper at `7d5e89e` and **11 failed**, while both control groups (capped-with-positive-total, uncapped-zero) passed unchanged — the assertions bite on the inconsistency and nothing else. With the correction the file is **51 passed / 0 failed** (was 37). Coverage added: the parser reason for both transport shapes; the real worker with an **empty actionable queue**; the real worker with **one actionable row that is cleaned and reconciled**, asserting malformed backlog evidence does not prevent legitimate cleanup; both shapes added to the generic malformed sweep, which already runs every shape with and without actionable rows; and the two controls above, retained and extended to both shapes.
>
> **REVISED CANDIDATE PACKAGE (still NOT deployed).** `badee27290890d85bc5f5a3f96ac64bb781ba25938375960577fe3f0a2190302` is **superseded**, as are `f69c450a…` and `8612c4b6…`. `index.ts` is byte-identical to pass 13b.
>
> ```
> 4f47f645e015879b5a7c2da62902a9bbab2dd2f12f768b501c4266cfd113ff64  functions/recording-retention-purge/index.ts       8,436 bytes
> ec50548e16f872aec932dc627987289c886dba66e1a08b3790de6ac0fb04cde6  functions/recording-retention-purge/voicemail.ts  62,036 bytes
> ```
> **manifest `bc1f163b4623a5c842253fb726eabe1abf19fb9617c0d77e6194210cecab261f`**, entrypoint `functions/recording-retention-purge/index.ts`, `verify_jwt = false`, no import map, exactly two files.
>
> **EVIDENCE — NEWLY EXECUTED this pass.** Focused worker regressions **192 passed / 0 failed** (`voicemailCleanupStarvation` **51**, `recordingRetentionVoicemail` 80, `voicemailOwnershipRecovery` 14, `voicemailOwnership` 16, `recordingBlankPath` 12, `recordingRetentionHandler` 6, `recordingRetentionBaseline` 6, `edgePayloadVerification` 7); the fail-first run described above; **type checking, with an earlier characterization corrected** — the root `npx tsc --noEmit` this document has been citing is **vacuous**: `tsconfig.json` is solution-style (`"files": []` plus `references`), so `--listFiles` emits **zero files** and exit 0 proves nothing. The project that actually covers both changed files is `tsconfig.app.json`, which carries a large pre-existing error baseline. Measured both ways: **88 errors at `7d5e89e`, 88 after this change, multiset identical** (a first draft of the new parser assertion added one, from `strictNullChecks: false` refusing to narrow a discriminated union on its literal tag; it was rewritten to read through a widened view and assert the reason unconditionally). `eslint` clean on both changed files; dependency closure rebuilt from the real entrypoint — exactly `index.ts` and `voicemail.ts` plus the single external `https://esm.sh/@supabase/supabase-js@2`; `edge_payload.mjs selftest` green (16 cases, offline serialization only) and `build` + `verify` round-tripped the serialized `files` argument against repository source, `PAYLOAD VERIFIED`.
>
> **EVIDENCE — CARRIED OVER from pass 13b, NOT re-executed this pass.** The SQL suites (`ALL INBOUND SQL SUITES GREEN (M1-M3 + v2 M4-M9, incl. both rollback proofs)`), the generated-types verification through M9 with its four negative controls, and the rollback proof with its six negative controls. None of those inputs changed: M8, M9, every SQL test, `src/integrations/supabase/types.ts`, `scripts/run_cp13_rollback_test.sh` and the type-generation scripts are byte-identical to `7d5e89e`, and the migration hashes are unchanged — M8 `4324c67981b20b4e3c33fd20ccb3618a6c70ba16fbb0ad796f0956858dfeb186`, M9 `0442669072c44a8ee8c3b3ceeadeff5fd25264a446e82baa9164aa2dca7f6583`. Carried-over evidence is stated as carried over; it was not re-run and is not claimed as fresh.
>
> Nothing in routing, recording-status, retention periods, existing RLS policies or grants, or unrelated application code was reopened, and no broad audit was run.

### Step 9 — Corrective pass 13b — **DEVELOPMENT ONLY; NOTHING DEPLOYED OR MIGRATED (2026-09-17)**

> Four follow-up defects, each found by executing the code rather than reading it. Production is untouched; M8, M9 and the corrected worker remain unapplied and undeployed, and **v30's failed byte-for-byte record is preserved unchanged**.
>
> **1. The blocked-summary parser fabricated zeros.** `normalizeBlockedSummary` defaulted anything missing or malformed to `0`, so an empty object `{}`, a summary missing `blocked_total` while reporting 100 due rows, non-numeric or negative counts, and self-contradictory counts (100 due out of a total of 0) each produced `completed / reason: no_work / queue_empty: true / blocked_summary_unavailable: false` — destroying the very unknown-versus-none distinction the blocked reporting exists to make. **Its own test asserted that zero-defaulting**, so the suite was blessing the defect. Replaced by `parseBlockedSummary`, which validates the COMPLETE contract and returns `{ok:false, reason}` on any breach: the singleton result shape (an array must hold exactly one row), every count present and a finite non-negative **integer** (a string, float, NaN, Infinity, null or absent key is a broken contract, not a variant), a strictly boolean `scan_capped`, an `oldest_blocked_at` that is present and `string | null`, and the relationships the SQL guarantees (`due ≤ total`, `orgs ≤ total`, `total > 0 ⇒ orgs ≥ 1`, `total = 0 ⇒ due = orgs = 0` and `oldest` null exactly when `total` is 0). An unknown summary sets `blocked_summary_unavailable = true`, keeps `queue_empty = false`, cannot yield `completed / no_work`, and **still lets actionable cleanup run** — observability never gates the work. The incorrect expectation is replaced; 23 parser cases and 16 worker-level regressions (each malformed shape **with and without** actionable rows) now run through the real worker, alongside retained valid-zero, valid-backlog, capped-scan and failed-read controls.
>
> **2. Generated-types verification stopped at M7.** `verify_inbound_generated_types.sh` built its isolated schema only through M7 and `emit_check.mjs` listed neither M8 RPC, so a green result verified nothing about the new contracts. The build now applies **M8 and M9**, and the comparison covers `voicemails_cleanup_actionable_batch` and `voicemails_cleanup_blocked_summary` Args/Returns. **The schema verified is `public`, and only `public`** — the generator runs with `included_schemas=public`, so `auth`, `storage` and `private` objects are outside this check; a green result is not whole-database coverage. M9's trigger function is handled by the generator's ACTUAL behaviour: `postgres-meta` does not emit trigger-returning functions into `Functions`, and the script now **asserts that absence** rather than inventing a client-callable contract for it. The repository type entries were regenerated from the isolated schema with the pinned generator (`@supabase/postgres-meta@0.96.1`, the version the pinned CLI ships) — never from production. A new `verify_inbound_generated_types_negative.sh` proves the check bites: omitting a new RPC, changing its Args type, changing a Returns type and dropping a Returns field are each **rejected**, and the unmodified repository file passes. The rule that any migration-application failure ABORTS verification is unchanged.
>
> **3. The rollback proof's assertions did not hold.** Its grant check was `*authenticated:SELECT*|*authenticated:UPDATE*`, which passes on **either** privilege — a stack with only SELECT, i.e. the browser unable to record a listen at all, passed it. Its policy check compared only names and commands, so a policy widened to `USING (true)` still passed. And it captured M7's function digest **after** applying M8/M9, so it could not prove the migrations left that function alone. Rewritten: an INVARIANT fingerprint captured **before either migration is applied** and compared after apply, after rollback and after two reapplications, covering complete policy definitions (permissiveness, roles, command, **both** expressions), RLS **and** force-RLS state, **exact** table and column privileges rendered as a set, and M7's original function by identity **and** definition. Catalog rendering is pinned with `COLLATE "C"` everywhere. Six **negative controls** on their own disposable databases prove it detects a missing `UPDATE (listened_at)` grant, an unexpected extra privilege, a policy broadened to `true`, a policy that gains a role, RLS being disabled, and M7's original function being changed. No production mutation at any point.
>
> **4. Object count and error-code corrections.** M8 adds **four** objects — two functions and two partial indexes — not five; the rollback comment and the table above are corrected, and the rollback proof now asserts the count. And an unavailable RPC through PostgREST **may** surface as a schema-cache code (`PGRST202` → `schema_inconclusive`) rather than PostgreSQL's `42883` (→ `schema_unavailable`); both are skips that do no work, and the promise that 42883 always appears has been removed from both migration files and from this document.
>
> **REVISED CANDIDATE PACKAGE (still NOT deployed).** Recomputed after the worker change; `f69c450a…` and `8612c4b6…` are both superseded.
>
> ```
> 4f47f645e015879b5a7c2da62902a9bbab2dd2f12f768b501c4266cfd113ff64  functions/recording-retention-purge/index.ts       8,436 bytes
> d67bb2883e87bffd9522b00bb215f373d3a511a4e199d7914c511b6ce0f9356d  functions/recording-retention-purge/voicemail.ts  60,641 bytes
> ```
> **manifest `badee27290890d85bc5f5a3f96ac64bb781ba25938375960577fe3f0a2190302`**, entrypoint `functions/recording-retention-purge/index.ts`, `verify_jwt = false`, no import map, exactly two files. Dependency closure re-verified: exactly these two local files plus `https://esm.sh/@supabase/supabase-js@2`.
>
> **EVIDENCE — fresh this pass.** Focused suites **178 passed / 0 failed** (`voicemailCleanupStarvation` 37, `recordingRetentionVoicemail` 80, `voicemailOwnershipRecovery` 14, `voicemailOwnership` 16, `recordingBlankPath` 12, `recordingRetentionHandler` 6, `recordingRetentionBaseline` 6, `edgePayloadVerification` 7) — the starvation and first-listen protections are intact, and purged rows still stay purged. `ALL INBOUND SQL SUITES GREEN (M1-M3 + v2 M4-M9, incl. both rollback proofs)`. Generated-types verification green through M9 with its four negative controls. Rollback proof green with its six negative controls. `tsc --noEmit` and `eslint` clean. Nothing in routing, recording-status, retention periods, existing RLS policies/grants or unrelated application code was reopened.

### Step 8 — Corrective pass 13 — **DEVELOPMENT ONLY; NOTHING DEPLOYED OR MIGRATED (2026-09-17)**

> **THREE CORRECTIONS TO THIS DOCUMENT'S OWN EARLIER CLAIMS, first.**
>
> 1. **"Not a regression against v29" does not close either defect.** It is true that deployed v29 contained no voicemail code, so neither defect can be blamed on this package relative to it — and that is beside the point. Both defects are in the **newly introduced voicemail workflow itself**. A new subsystem is not correct because the subsystem it replaces did not exist; the comparison says something about blast radius and nothing about correctness. Both are treated below as open defects, not as accepted deltas.
> 2. **Zero current voicemails establishes no immediate exposure, not future correctness.** The earlier entries were right that `voicemails` holds 0 rows, so neither defect can bite today. They were wrong to let that stand as reassurance about the design. The starvation defect activates the moment v2 produces voicemails whose owner is not established, and the timestamp defect activates the moment any voicemail exists for an authorized caller to stamp. An empty table is a statement about timing, not about whether the code is right.
> 3. **The stripped comparison was in CHARACTERS, not bytes.** §2 Step 7 recorded "38,426 bytes identical on both sides". Re-measured: the comment-stripped content of deployed v30's `voicemail.ts` is **38,426 characters = 38,508 UTF-8 bytes**. The conclusion it supported (v30's executable content is identical to the approved package) is unchanged; the unit was wrong and is corrected here rather than quietly restated.
>
> **v30's record is preserved exactly as it was.** Its manifest is still `ce4d4491…` against an approved `8612c4b6…`; its failed byte-for-byte verification is not amended into a pass, and the 10 dropped U+2500 characters across 8 comment lines remain recorded as a failure. The corrected package below is a NEW candidate with NEW digests — it does not retroactively bless v30.
>
> **DEFECT 1 — cleanup starvation (M8 + worker).** A row whose `provider_account_sid` cannot be established is deliberately left completely untouched — no request, no failure record, no attempt increment, no backoff — so it can neither be deleted against the wrong account nor silently retired at M7's 50-attempt ceiling. But M7's `voicemails_cleanup_batch` selects `ORDER BY created_at ASC LIMIT 100` with no owner filter, so such rows are immortal at the head of the queue: every batch returns the same prefix, the in-pass no-progress guard stops the invocation, and actionable rows behind them are never reached on any run. **The correction** is a second, narrower selection — `voicemails_cleanup_actionable_batch`, the same M7 predicate plus an established owner — with `voicemails_cleanup_blocked_summary` reporting the backlog that selection skips. No owner is ever guessed, blocked rows are never modified, no attempt is fabricated, and each becomes actionable again by itself the moment a signed callback persists its owner. It is **not** a bigger batch and **not** offset pagination: there is no blocked prefix left to page past, so nothing restarts at the same stuck rows next invocation. `queue_empty` now means *nothing is owed* — actionable queue drained AND no blocked backlog AND the backlog was readable — so excluding blocked rows can never be reported as an empty queue.
>
> **DEFECT 2 — the retention clock (M9).** The earlier description of this one was **overstated and is corrected**: access is not unrestricted. `can_access_voicemail` already limits both policies to the recipient agent, eligible group members, organization Admins and qualifying super admins within their own organization, and `authenticated` holds only SELECT plus a column-scoped `UPDATE (listened_at)`. The real defect is narrower and still real: an **authorized** caller supplied the VALUE, and `voicemails_expired_batch` trusted it, so a backdated timestamp brought forward an irreversible deletion — with no trigger or constraint preventing backdating, future-dating, clearing or resetting. The browser helper already documented "first listen wins"; that intent lived only in client code. **The correction** enforces it in the database: a `BEFORE UPDATE OF listened_at` trigger stamps the first accepted listen with `now()`, makes repeats idempotent, and restores an established value against any backdate, future-date, clear or reset. **One exemption is deliberate and named**: `service_role` (and `postgres`, so migrations and backfills) may still set the column — it already holds GRANT ALL on this table, so guarding it would add friction without protection. A test asserts that **no shipped function writes `listened_at`**, so the exemption stays honest. **This records when the system ACCEPTED a listen event from an authorized caller — it does not prove a human heard the audio.**
>
> **No RLS policy, grant or role membership was changed**, and the retention rules are untouched: heard voicemail still expires on the first-listen timestamp, unheard voicemail on the existing 90-day `created_at` cap.
>
> **NEW MIGRATION SCOPE — both UNAPPLIED everywhere.** M4–M7 were not edited.
>
...
> | `20260918002859_voicemail_first_listen_guard.sql` (M9) | `voicemails_enforce_first_listen()` + the `voicemails_first_listen_guard` trigger | `rollback/20260918002859_…` drops both |
>
...
>
> **CORRECTED PACKAGE (candidate — NOT deployed). The old approved manifest no longer describes it.**
>
> ```
> 4f47f645e015879b5a7c2da62902a9bbab2dd2f12f768b501c4266cfd113ff64  functions/recording-retention-purge/index.ts       8,436 bytes
> ed7fa634f9bd4ea29ab9d74148625480594b81230906361f7db9c9857aa97a2c  functions/recording-retention-purge/voicemail.ts  56,217 bytes
> ```
> **manifest `f69c450a6910f0a0e7c1efc3e106055a183d17f572fe4fa699d76c7f42b7693d`**, entrypoint `functions/recording-retention-purge/index.ts`, `verify_jwt = false`, no import map, exactly two files. **`8612c4b6…` is SUPERSEDED** and authorizes only the older package. Dependency closure re-verified: exactly these two local files plus `https://esm.sh/@supabase/supabase-js@2`.
>
> **RELEASE ORDER, and why it is safe in this order.**
> 1. **Apply M8.** Safe against the CURRENTLY DEPLOYED worker: M8 only adds objects and leaves M7's `voicemails_cleanup_batch` byte-identical, which the rollback proof asserts explicitly. v30 keeps calling M7's function and keeps behaving exactly as it does today, starvation included.
> 2. **Deploy the corrected worker** (separate approval), which calls the two M8 functions and no longer calls M7's. If the order were reversed the cleanup phase would report `skipped` and do no work — visible and non-destructive, but pointless. **Which `reason` appears is not guaranteed**: PostgreSQL's own SQLSTATE 42883 classifies as `schema_unavailable`, while a PostgREST **schema-cache** miss (PGRST202) classifies as `schema_inconclusive`. Both are skips; the earlier promise that 42883 always appears was wrong, because the API layer's cache state decides.
> 3. **Apply M9** at any time. It is independent of every Edge function: nothing calls it, and no deployment has to accompany it.
>
> **ROLLBACK COMPATIBILITY.** M9's rollback is unconditional and restores M7 behaviour exactly — which reintroduces the defect, so it is a deliberate act. Timestamps already stamped are ordinary values and are left alone; the rollback reads and rewrites nothing. **M8's rollback has an ordering requirement**: return the worker to a build that calls `voicemails_cleanup_batch` FIRST, or the cleanup phase will report `skipped` on every run and do no cleanup — `schema_unavailable` or `schema_inconclusive` depending on whether PostgreSQL's 42883 or a PostgREST cache miss surfaces; visible and non-destructive either way. Neither rollback touches voicemail rows, obligations, counters, media or notification state, and neither touches policies or grants (asserted).
>
> **TEST EVIDENCE — fresh this pass unless marked carried over.**
> * **SQL, executed as the REAL `authenticated` role** (`supabase/tests/voicemail_listen_guard_and_cleanup_selection.sql`, wired into the runner): L0 reproduces the timestamp defect FAIL-FIRST by disabling the guard — the forged 2000-01-01 value sticks and `voicemails_expired_batch` immediately offers fresh media for deletion; L1 server-stamping; L2 backdate/future-date/clear/reset all refused and idempotent; L3 no side effects on any other column; L4 recipient, group member and org Admin allowed, unrelated same-organization agent and cross-organization Admin denied with nothing written; L5 the forged clock cannot prematurely expire fresh media while the heard branch and the 90-day unheard cap still work; L6 `authenticated` can neither INSERT nor DELETE a row to smuggle a timestamp in; L7 the service_role exemption, both halves; L8 no shipped function writes `listened_at`. C0 reproduces the starvation FAIL-FIRST on identical data (M7's batch returns 100 blocked rows and never the actionable one; the actionable selection reaches it); C1–C7 malformed owners, blocked reporting with untouched obligations, cross-organization, owner recovery, the preserved M7 contract, the honest scan cap, and service-role-only privilege.
> * **Rollback proof** (`scripts/run_cp13_rollback_test.sh`, its own throwaway database): M4–M7 baseline is 0/0/0 → M8+M9 apply 3/1/2 → both rollbacks return 0/0/0 → reapply twice (idempotent). Asserts M7's `voicemails_cleanup_batch` is byte-identical throughout and that policies and the `listened_at` grants never drift.
> * **Whole inbound SQL runner GREEN** end to end: `ALL INBOUND SQL SUITES GREEN (M1-M3 + v2 M4-M9, incl. both rollback proofs)`.
> * **TypeScript**: `voicemailCleanupStarvation.test.ts` (20 new) runs the REAL `runVoicemailSourceCleanup` over a model faithful to the live SQL — fail-first at 100 and at the 99 boundary and across three successive invocations; corrected at 100, at 1,200 blocked rows, cross-organization, repeated invocations with no fabricated attempts, owner recovery, and the preserved timeout / bounded concurrency / capacity ceiling / reconciliation / counter behaviour; plus the blocked summary as observability that never gates the work. `edgePayloadVerification.test.ts` (7 new). The callback-to-worker ownership-recovery suite still passes, **purged rows still staying purged**.
> * **Full vitest: 2,659 passed, 0 failed** (2,632 at `d312b80` + the 27 added). Eleven test FILES fail to load for want of browser Supabase env vars — **verified pre-existing** by re-running at clean `d312b80`, and none of them is a file this pass touched. `tsc --noEmit` and `eslint` clean.
> * **Carried over, not re-executed**: the broader M4–M7 SQL suites' original evidence and the earlier corrective passes' test totals.
>
> **THE DEPLOYMENT-PAYLOAD HAND-OFF (§item 5) — tooling, and a CORRECTED description of what is actually limited.** v30 failed because the pre-submission check validated a FILE ON DISK and the payload was then transcribed a SECOND time into the deployment call; that second transformation — source text into a JSON-escaped string literal — was never checked. `scripts/edge_payload.mjs` now **builds the exact `files` argument** and **verifies a candidate serialized payload** against the repository byte for byte, naming the first differing line; its regression test injects precisely the v30 defect (one dropped U+2500) and requires a non-zero exit; its offline self-test covers leading and trailing newlines, absent trailing newline, interior blank lines, double quotes, single quotes and backticks, backslashes, tab/CR, em dash, both repeated box-rule widths, adjacent mixed dashes, astral-plane and control characters — and is **labelled offline**: it proves JSON round-tripping, **not** a deployment.
>
> **CORRECTION.** rev 20 said the remaining gap was that "MCP `deploy_edge_function` accepts `files[].content` only as an inline string — it cannot read a path — so closing it needs a different channel such as the Supabase CLI". **That was wrong, and it misattributed the limitation.** `deploy_edge_function` takes `files` as a **structured argument**, and the file this tool writes *is* that argument value: a programmatic caller reads the verified JSON, parses it, and passes the resulting array straight through. No source reconstruction, no retyping, and **no different deployment channel, CLI, path-based tool or credential extraction is inherently required** — the existing Supabase MCP accepts the resulting object. **The residual limitation belongs to the CALLING ENVIRONMENT, not to MCP**: in a harness where the caller must author tool-call arguments as literal text in its own output (this Claude Code session is one), a tool argument cannot be bound to the result of a file read, so that last hop still passes through authored text. In a caller that can pass a parsed value directly, it does not. The eventual production hand-off belongs to a **separately approved deployment step**.
>
> **REMAINING LIMITS, unchanged by this pass.** Admission budgets are not hard cancellation deadlines. The conversation-recording pass remains unbounded and runs first. Cleanup capacity stays capped at 10 batches × 100 rows, concurrency 5. Unresolved ownership still preserves the provider source. Phase outcomes are still visible only through logs and the response body — no monitoring integration exists. M7's pg_cron-present rollback remains unproven. The voicemail FK/CHECK contradiction stands as a hard-deletion limitation. **Alexa's incident remains unverified** until controlled live testing demonstrates audible background-tab ringing and correct routing.
>
> **BOTH DEFECTS ARE GATES ON v2 ACTIVATION.** Neither is closed by the fact that production holds no voicemails, and neither is closed by v29 having had no voicemail code. v2 must not be activated until M8 and M9 are applied and the corrected worker is deployed and verified.

### Step 7 — `recording-retention-purge` — **DEPLOYED AT v30 (2026-09-16 22:54:33 UTC); BYTE-FOR-BYTE VERIFICATION FAILED; DEPLOYMENT WRITES STOPPED**

> **OUTCOME, STATED FIRST AND PLAINLY.** One `deploy_edge_function` call answered **version 30**, ACTIVE, `verify_jwt = false`, `import_map = false`, `ezbr_sha256 6a4acf05c624bd089a0efcfd5407896a1cbfb7bcd56af0b151961128a7edab21`, deployed **2026-09-16 22:54:33 UTC** (v29's record: 2026-05-22 19:22:39 UTC, preserved). The response was **not** treated as verification. An independent `get_edge_function` readback was hashed directly from the returned bytes and **`voicemail.ts` did not match the approved digest**:
>
> ```
> file          approved                                                          deployed (v30)
> index.ts       7,935 B  7db40c798d15f1a903ef1c862148897433dfa0cfb6a523ef246e7ffddeb688e2   IDENTICAL
> voicemail.ts  48,637 B  2a0cbaa1417cbe7c122772ad45ef1d3a027bfd3fa0ad47f3f2e96e64f9646ad3
>               48,607 B  efe6187e57a9cb56ed58bc42620d9403b00cc232ed55d45c45d245287c0a13d0   DIFFERS (-30 bytes)
>
> canonical manifest from readback  ce4d44918ab0edfeb903623fbb45eec7c75efc4321b7e1fe0d0c766fa70e553e
> APPROVED manifest                 8612c4b6d0d5705f9da7bfefa5960ba517596fb8f01071b13afb5cbe27a84b6b   NO MATCH
> raw-path manifest from readback   26cd3760f0d782093a59164effb8158ee1b9b899ff9071573afce859fe7eeeda   (kept distinct)
> ```
>
> **The exact difference.** Eight lines differ, all of them decorative section rules that are whole-line `//` comments: lines **76, 181, 313, 428, 572, 608, 799, 998**. Each lost one `─` (U+2500 BOX DRAWINGS LIGHT HORIZONTAL) except line 799, which lost three — **10 characters x 3 UTF-8 bytes = the 30-byte shortfall**. Line count is identical (1,072). Mechanically established: every differing line begins with `//` on both sides; stripping all whole-line comments leaves **38,426 bytes identical on both sides**; and each difference is *only* a change in the count of U+2500, with the non-padding text equal. **The executable content of v30 is therefore identical to the approved package** — but executable equivalence is explicitly **not** a substitute for byte-for-byte verification, so this deployment **does not satisfy its approval**.
>
> **Where the defect entered.** The validated payload file written during Step 4 is byte-identical to the approved source (`2a0cbaa1…`, re-verified after the failure). The 10 characters were dropped when that verified file was transcribed into the deployment call — a long run of identical characters was reproduced short. The Step 4 pre-submission check **did not catch it**, because it validated the file on disk rather than the string actually submitted; that gap is the lesson from this deployment.
>
> **What was NOT done, per the approval.** No retry, no second deployment call, no switch of deployment mechanism, no rollback to v29, no schema/RLS/RPC/secret/setting change, no manual purge invocation, no generated callback, no placed call, no seeded data, no schedule change. The v29 baseline is preserved and re-verified (3,647 bytes, `96a21140728ac2ca97cca79a4a956e2b8bb236f8449535f92d4eec221ddc19eb`, ezbr `149f4473…`), and deployed v29 is confirmed to contain **no Twilio-source cleanup and no voicemail code at all** (zero matches).
>
> **Prerequisites confirmed read-only before deploying (all fresh this pass).** M4–M7 applied at their recorded versions with **nothing newer**. `voicemails` bucket **private**, `26 214 400` bytes, `audio/mpeg`. All five RPCs — `voicemails_expired_batch`, `mark_voicemails_purged`, `voicemails_cleanup_batch`, `mark_voicemail_source_deleted`, `record_voicemail_cleanup_failure` — present with their reviewed signatures, `SECURITY DEFINER`, `search_path=pg_catalog, pg_temp`, and **service_role-only EXECUTE** (plus the postgres owner). Their **bodies were proven unchanged**, not assumed: `md5(prosrc)` for all five equals the md5 of the corresponding body extracted from `20260915053646_inbound_voicemails.sql`. **None was invoked.** The cron job `recording-retention-purge-daily` is active at `15 8 * * *`, postgres/postgres, command digest `0af91853bb394a05addcbc279de42ff1`, length 369 — unchanged before and after, and its command was never printed.
>
> **Refreshed due-set counts, not assumed fixed.** At **2026-09-16 22:17:04 UTC** the conversation-recording due set was **32 rows** (7-day retention, one organization) — up from the 20 observed at 20:40 UTC the same evening; `calls` had grown 1,869 → 1,909. After the deployment, `call-recordings` held **242** objects against **242** rows with a storage path. The voicemail side is entirely empty: **0** voicemails, **0** bucket objects, **0** retention-due, **0** cleanup-due, **0** route attempts, **0** v2 calls, engine `legacy`, `voicemail_enabled = false`, 30-day listened retention.
>
> **Package verification that DID pass (Step 3, fresh).** Both approved digests and the manifest `8612c4b6…` matched at HEAD `955ce72`; exactly two files; **0 lines removed or modified** versus retrieved v29, and reconstructing v29 by dropping only the added lines reproduces it byte-for-byte; the **authentication region** (`0d29d23d…`) and the **conversation-recording purge loop** (52 lines, 1,759 bytes, `467bf70d…`) are byte-identical to v29; the only insertions are the helper import, the `invocationStartMs` capture, an explanatory comment, the `runVoicemailPhases` call, the response spread and `buildVoicemailDeps`; **104/104** focused purge tests pass (`recordingRetentionVoicemail` 80, `recordingRetentionHandler` 6, `recordingRetentionBaseline` 6, `recordingBlankPath` 12); and the dependency closure is exactly the two local files plus `https://esm.sh/@supabase/supabase-js@2`. **Carried over, not re-executed this pass:** the broader SQL suites and the wider 166-test totals from the earlier corrective passes — they apply to the same bytes, since the purge package is unchanged since then.
>
> **Scheduled-execution implications.** v30 is live and the daily job fires at **08:15 UTC**. Deployed `index.ts` is byte-identical to the approved file, so the conversation-recording pass that runs first is exactly v29's, and it will clear whatever has aged past 7 days by then (at least the 32 rows already due, plus anything that ages in overnight) and remove the matching `call-recordings` objects. With **zero** voicemails both new phases have nothing to do; the expected shape is `voicemail_retention` `completed`/`queue_empty` and `voicemail_source_cleanup` `completed` with `reason: "no_work"`. **A cron "succeeded / 1 row" result proves neither the Edge response nor voicemail processing**, and HTTP 200 with top-level `ok:true` describes only the conversation-recording pass — `voicemail_phases_ok` and the per-phase statuses, reasons and counters are the signal to read.
>
> **Runtime behaviour of v30 is UNPROVEN.** `recording-retention-purge` shows exactly **one** invocation in `function_edge_logs` across the 24-hour window, at UTC hour **8** — this morning's scheduled run, which executed **v29**, hours before v30 became ACTIVE at 22:54:33 UTC. **No invocation has reached v30**, and none was generated: the function was not invoked manually, no callback was produced, no call was placed, no data was seeded. Missing or absent log rows are reported as such and are not treated as proof of zero invocations.
>
> **Unchanged after the deployment, checked explicitly.** `twilio-recording-status` **36** (`a75e7c80…`), `twilio-voice-status` **42** (`d9bbe55c…`), `twilio-voice-inbound` **44** (`4e0b95da…`), `inbound-call-claim` **38** (`d6c5a82b…`) — all ACTIVE, `verify_jwt = false`, prior bundle hashes intact. Routing settings, retention settings, the voicemail bucket and the cron job are all unchanged; no migration newer than M7 exists.
>
> **Documented limits that remain in force.** Admission budgets are **not** hard cancellation deadlines — already-started work finishes within its own bounds, so an invocation may overrun by up to one unit of admitted work. The existing conversation-recording pass remains **unbounded** and runs first. Cleanup capacity is capped at 10 batches x 100 rows with concurrency 5. Unresolved ownership **preserves** the provider source and records nothing. Phase outcomes are visible only through logs and the response body; no monitoring integration exists yet.
...
**Evidence.** **166 focused tests pass across 8 files.** Measured reproductions, each shown failing against the source it corrects: the four 411faf4 cleanup defects (via a fixture extracted mechanically from that commit, its fidelity re-proved on every run); the account-aware ownership hole, with correct-owner 204/404 positive controls alongside NULL, malformed and conflicting owners; the 5,001-row retention capacity probe and budget-only exhaustion; **the real purge handler executed end to end** — `index.ts` and `voicemail.ts` bundled with the esbuild binary and run under Node with the Deno registration/environment, database and fetch supplied as adapters — where a 120 s first read now issues **zero** DELETEs while **the same harness against 0707038 still issues one**; and **the real recording-status callback executed with genuine HMAC-SHA1 signature validation**, driving a database double that models the PostgREST write boundary and then handing the same modelled table to the real purge worker, where the callback now persists account B and the next worker run completes the deletion — while **the same sequence against 42b48df leaves the owner NULL and the worker stuck**. Retained in full: the existing cleanup, retention and recording-callback coverage, plus M7's **46-assertion SQL suite V1–V10** on a disposable PostgreSQL 16.13. `tsc --noEmit` and `eslint` clean; **both** dependency closures built from the real entrypoints, each exactly its two package files plus `https://esm.sh/@supabase/supabase-js@2`. **Deno and the Supabase CLI remain absent**, so `deno check` and a local `functions serve` still could not run — the handler harness is the substitute, and it executes the genuine source.

...
**Source commit:** `35f4e3ff20f4ed3fd514c011fd7ff29c95b7e591` (branch `claude/agentflow-inbound-plan-fkl6zi`, 13 commits ahead of `main` `1b93f89f990b1482d90bf935633594c2851b8da8`, clean working tree).
**Prepared:** 2026-09-12 · **Status: PREPARATION ONLY.** Nothing in this document has been executed. Every step needs its own approval.
...
Migrations are applied with the Supabase MCP `apply_migration` tool (or `supabase db push`) against project `jncvvsvckxhqgqvkppmj`, one file per approval. Edge Functions are deployed with `supabase functions deploy <slug> --project-ref jncvvsvckxhqgqvkppmj` (or the MCP `deploy_edge_function`), from commit `35f4e3f`.
...
- **Prerequisites.** §1.5 dashboard confirmation not required for a direct apply (only for a merge); current `pg_dump`-equivalent backup point noted; nobody mid-call (advisory, not required — the migration takes no locks on `calls`).
- **Success checks (read-only).** Both tables exist with `relrowsecurity = true`; policy counts 4 and 2; the two functions exist; `\d` shows no change to `calls`/`profiles`; the applied-migrations list gains `20260911000100`.
- **Recovery.** `supabase/migrations/rollback/20260911000100_...rollback.sql` drops both tables and both functions. Nothing else references them at this point, so rollback is unconditional.
...
All of the following were executed on this branch at `35f4e3f` and need no repetition unless the code changes: the 10 SQL suites plus both two-session proofs, all six three-session barrier proofs and the M7→M6→reapply rollback proof; `scripts/verify_inbound_generated_types.sh`; `tsc --noEmit` exit 0; the app-config error set byte-identical to `main` (81); eslint clean on touched files; four Edge bundles; the full vitest suite (2510 passing, the same 11 environment-only baseline failures as `main`); `npm run build`.
...
| Needed | Why | Who |
|---|---|---|
| Dashboard read of Project Settings → Integrations → GitHub | Confirm "Deploy to production" is still OFF, so a merge cannot auto-apply M4–M7 (§1.5). Not readable from any tool available here | Chris |

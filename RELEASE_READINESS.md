...
**Prepared:** 2026-09-13 (rev 6) · **Updated:** 2026-09-15 (rev 11) · **Status: ALL FOUR MIGRATIONS M4–M7 ARE APPLIED, and `twilio-voice-status` IS DEPLOYED AT v41 WITH A FAILED BYTE-FOR-BYTE PACKAGE CHECK (one dropped newline; executable output proven identical — see §2 Step 5). Everything else is still PREPARATION ONLY** and needs its own approval.
...
### Step 5 — `twilio-voice-status` — **DEPLOYED 2026-09-15 as v41; PACKAGE VERIFICATION FAILED ON ONE BYTE**

> **Outcome.** Approved by Chris on 2026-09-15 from `cb4fe67`→`95dcc46e4a4acffeb89017c61c3f302a52fcee5d`, package manifest `9d8540cb…d8aa174f`. One `deploy_edge_function` call. The function is **ACTIVE at version 41**, `verify_jwt = false`, entrypoint `functions/twilio-voice-status/index.ts`, `import_map = false`, `ezbr_sha256 = 16715618da1209a7a5f40e535a02b8c6e0bb73326027f993679d049e33f96795` (baseline v40 was `4d2425e2…44e5aef1`).
>
> **The byte-for-byte check did NOT pass.** Four of the five files are byte-identical to the approved package. `functions/_shared/notifications.ts` deployed at **7 701 bytes against the approved 7 702** — the deploy payload I composed dropped the file's **leading blank line**. Proven exactly: the approved file *minus its first byte* equals the deployed file with no other difference. The deployed manifest is therefore `d527b53fb59408326cb484dc5e7dee66c801c7f037e6645d4aa957221f505995`, **not** the approved `9d8540cb…d8aa174f`.
>
> **The defect is inert but the contract is still unmet.** Bundling the deployed package and the approved package with esbuild and normalising only esbuild's file-path banner comments yields **byte-identical output** (`20cc1876312790244b37ff0f3f35ec866b709da6f98500832b987c9e11c6196a` both sides), so v41's executable behaviour is exactly the approved behaviour. That does not satisfy "every deployed source file matches the approved package byte for byte" or "the package manifest matches the approved digest", so this step is recorded as **FAILED VERIFICATION**, not as a pass.
>
> **No further deployment write was made.** Per the approval, an uncertain outcome or failed verification stops deployment writes and reconciles read-only: there was no redeploy, no rollback, no secret change and no authentication change. Correcting v41 forward to a byte-exact v42 requires a separate explicit approval. **Precedent:** the same class of defect occurred on `twilio-voice-inbound` v43 (81 comment-only bytes lost to miscounted `\u2500` escapes in the deploy payload) and was corrected forward to v44 under Chris's explicit approval rather than by restoring baselines; executable code never differed there either.

**Verified at deployment time.** M4–M7 all applied (`20260914000530` / `20260915025931` / `20260915035141` / `20260915053646`); all eight `calls` columns the new projection selects exist; `converge_inbound_notifications(p_call_row_id uuid)` present, `SECURITY DEFINER`, body `24db4e0f…`, **`service_role` EXECUTE only** (the handler runs on the service-role key); `uq_notifications_user_event_key` present; every organization still `legacy` with an empty group (`3bfd8969…`). The other three release functions are **unchanged** at `twilio-recording-status` 34, `recording-retention-purge` 29, `twilio-voice-inbound` 44. No application data changed: `calls` 1 862 with `max(updated_at)` still 2026-09-14 21:30:10, notifications 98 (`380f661b…`), voicemails 0, attempts 0, and the M7 sweeps still 308/308 succeeded with zero failures.
...
**Checks that could not be run, stated plainly.** **Deno is not installed** in this environment, so no `deno check` of the entrypoint was possible; the Supabase CLI is absent, so no local `functions serve` smoke test. Substitutes actually run: an esbuild bundle proving the module graph resolves with the dependency closure equal to exactly the five approved files and `https://esm.sh/@supabase/supabase-js@2` as the only external, `tsc --strict` clean on the four Deno-free modules, and 8 focused test files / **118 tests all passing** (legacy recipients, D13 snapshots, unresolved v2 recipients, retryable DB failures → 503, callback convergence, terminal-state freezing, duration monotonicity). **No runtime evidence of v41 exists:** `function_edge_logs` holds 7 invocations for this function, all on 2026-09-14 between 18:09 and 21:30 UTC — before the deployment — so no natural callback has reached v41. That is a gap in observation, not a defect, and it is separate from the package verification above. Live routing, ringing and callback behaviour remain unproven until the controlled tests.

### Steps 6–8 — the remaining Edge Functions, in this order
`twilio-recording-status` → `recording-retention-purge` → `twilio-voice-inbound`.
...
- **Success checks.** Each function reports a new version (`twilio-voice-status` reached **41** on 2026-09-15; then 35 / 30 / 45 respectively); place **no** live call; watch the function logs for one legacy inbound call handled end to end with unchanged behaviour (identity resolution, ring, status callback, recording) and no reference to a v2 table; `recording-retention-purge` next scheduled run at 15:08 UTC completes with the recording count unchanged apart from genuine expiries.

...
**Prepared:** 2026-09-13 (rev 6) · **Updated:** 2026-09-17 (rev 20) · **Status: production UNCHANGED — `recording-retention-purge` v30, `twilio-recording-status` v36, `twilio-voice-status` v42, `twilio-voice-inbound` v44, `inbound-call-claim` v38, every organization on legacy. v30's byte-for-byte verification FAILURE still stands and is NOT retroactively passed (§2 Step 7). Corrective pass 13 is DEVELOPMENT ONLY: two new UNAPPLIED migrations (M8, M9), a corrected purge worker, and a payload verifier — nothing deployed, nothing migrated.** **Both voicemail defects are GATES ON v2 ACTIVATION** until their fixes are applied and verified (§2 Step 8).
...
> |---|---|---|
> | `20260917010000_voicemail_cleanup_actionable_selection.sql` (M8) | `voicemails_cleanup_actionable_batch(integer)`, `voicemails_cleanup_blocked_summary(integer)`, two partial indexes | `rollback/20260917010000_…` drops exactly those five objects |
...
> 2. **Deploy the corrected worker** (separate approval), which calls the two M8 functions and no longer calls M7's. If the order were reversed, the worker would receive SQLSTATE 42883 and report the cleanup phase `skipped / schema_unavailable` — visible and harmless, but pointless.
> 3. **Apply M9** at any time. It is independent of every Edge function: nothing calls it, and no deployment has to accompany it.
>
> **ROLLBACK COMPATIBILITY.** M9's rollback is unconditional and restores M7 behaviour exactly — which reintroduces the defect, so it is a deliberate act. Timestamps already stamped are ordinary values and are left alone; the rollback reads and rewrites nothing. **M8's rollback has an ordering requirement**: return the worker to a build that calls `voicemails_cleanup_batch` FIRST, or the cleanup phase will report `skipped / schema_unavailable` on every run — visible, non-destructive, but no cleanup happens. Neither rollback touches voicemail rows, obligations, counters, media or notification state, and neither touches policies or grants (asserted).
>
...
>
> **THE DEPLOYMENT-PAYLOAD GAP (§item 5) — narrowed, and the remainder reported rather than hidden.** v30 failed because the pre-submission check validated a FILE ON DISK and the payload was then transcribed a SECOND time into the deployment call; that second transformation — source text into a JSON-escaped string literal — was never checked. `scripts/edge_payload.mjs` now **builds the exact `files` argument** and **verifies a candidate serialized payload** against the repository byte for byte, naming the first differing line. Its regression test injects precisely the v30 defect (one dropped U+2500) and requires a non-zero exit. Its offline self-test covers leading and trailing newlines, absent trailing newline, interior blank lines, double quotes, single quotes and backticks, backslashes, tab/CR, em dash, both repeated box-rule widths, adjacent mixed dashes, astral-plane characters and control characters — and is **labelled offline**: it proves JSON round-tripping, **not** a production deployment. **The remaining gap, stated plainly: the approved MCP `deploy_edge_function` accepts `files[].content` only as an inline string argument. It cannot read a path or a digest, so the final hand-off still passes through caller-authored text and cannot be mechanically tied to the verified file.** Closing that last step needs a channel that ingests a file — the Supabase CLI, or an MCP tool taking a path or digest — and **that requires its own approval; it is not assumed, and no credential was extracted, no channel switched and no further production call made to experiment.**
>

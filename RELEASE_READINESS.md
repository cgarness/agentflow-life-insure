...
**Prepared:** 2026-09-13 (rev 6) · **Updated:** 2026-09-16 (rev 18) · **Status: ALL FOUR MIGRATIONS M4–M7 ARE APPLIED, `twilio-voice-status` IS DEPLOYED AND FULLY VERIFIED AT v42, and `twilio-recording-status` IS DEPLOYED AND FULLY VERIFIED AT v36** — the v41 byte-for-byte failure (one dropped leading newline) was corrected forward under a separate approval and v42 now matches the approved package digest exactly (see §2 Step 5); `twilio-recording-status` reached v35 cleanly (§2 Step 6, **historical record preserved**) and v36 is a **corrective forward deployment** of the durable-ownership-recovery package, verified byte-for-byte (§2 Step 6b). **Everything else is still PREPARATION ONLY** and needs its own approval.

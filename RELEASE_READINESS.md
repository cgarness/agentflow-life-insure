...
>
> ```
> 9b6f8b8869f6afd4f9c18bb0aa24ab1642e80f7ea8d3feb6e8a286827170b251  functions/recording-retention-purge/index.ts        7,429 bytes
> 1d35507ddd8d5ba6c3d4a412aa837de7af52212538b206557e5e55ee434bc44c  functions/recording-retention-purge/voicemail.ts  41,159 bytes
> ```
> **manifest `ae8d4ba2a43ee16566f9076f64a2308fad309ab2599a1b1bb690d6d67fed7cb4`**, entrypoint `functions/recording-retention-purge/index.ts`, `verify_jwt = false`, no import map. The **old 9,002-byte source and manifest `c42c4006…d7c9e164` are SUPERSEDED** and authorize only the old package.
>
...
**Evidence.** **74 focused tests pass**: 6 reproducing the defects against a fixture extracted mechanically from 411faf4 (fidelity re-proved on every run), and 47 against the correction — returned-error and thrown-error paths for both cleanup RPCs, idempotent reconciliation, provider 2xx/404/HTTP-failure/network/timeout, a stalled row with later progress, 101 rows, an unchanged batch stopping after exactly 3 DELETEs rather than 6, budget exhaustion preserving obligations, and all four outcome shapes proven mutually distinguishable. Retained: the voicemail storage-removal check and the retention cutoffs, plus M7's **46-assertion SQL suite V1–V10** on a disposable PostgreSQL 16.13. `tsc --noEmit` and `eslint` clean; the real Edge dependency closure is exactly the two package files plus `https://esm.sh/@supabase/supabase-js@2`. **Deno and the Supabase CLI remain absent**, so `deno check` and a local `functions serve` could not run.
...
**Three the reviewer asks a human to weigh, not wave through.** (1) **A provider 404 is still proof of deletion** with a platform-account fallback: a recording in a subaccount whose SID was never stored is marked durably `deleted` while the media survives. Pre-existing and identical in 411faf4 and deployed v29, and it is this pass's approved contract — raised for a decision, not changed unilaterally. (2) **The new outcome surface is operationally invisible**: pg_cron discards the response body, so only the Edge Function logs carry it; nothing in this package adds an alert or a body-reading consumer. (3) **New capacity bound**: up to 1 000 DELETEs per nightly run at concurrency 5, Twilio rate-limit behaviour unverified, and a backlog above 1 000 rows/day can never drain on the unchanged daily schedule.

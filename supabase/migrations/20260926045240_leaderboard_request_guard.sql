-- Organization leaderboard concurrency guard. Production apply needs exact approval.
-- Accept only the verified paused state; never replay the historical pause.
...
BEGIN
  ...
  IF md5(original) <> '1314cefc781ff326540b83f748d48046' THEN
  ...
  IF (SELECT proowner <> 'postgres'::regrole
    ...
    RAISE EXCEPTION 'Leaderboard owner or ACL changed; refusing patch';
  END IF;
  ...
  END IF;
  IF md5(pg_get_functiondef(target)) <> '8af04a4deed619788ee803df90d59205'
     OR md5(replace(pg_get_functiondef(target),guard_marker,'')) <> 'd26a38b59de90db91ed777236ee4acc4' THEN

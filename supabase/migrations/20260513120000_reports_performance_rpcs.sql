-- Migration: Phase 3 Reports Performance RPCs
-- Adds 4 server-side RPCs for generating pre-aggregated report data

-- 1. rpc_report_call_summary
DROP FUNCTION IF EXISTS rpc_report_call_summary;
    ...
    WITH base_calls AS (
        SELECT c.*,
               (c.duration > 45 OR EXISTS (
                   ...
                   AND (d.name ILIKE 'dnc' OR d.name ILIKE 'do not call')
               )) as is_contacted,
               EXISTS (
                   SELECT 1 FROM dispositions d
...
-- 2. rpc_report_call_volume_timeseries
DROP FUNCTION IF EXISTS rpc_report_call_volume_timeseries;
...
-- 3. rpc_report_disposition_breakdown
DROP FUNCTION IF EXISTS rpc_report_disposition_breakdown;
...
BEGIN
    WITH base_calls AS (
        SELECT c.id, c.agent_id, c.campaign_id, c.duration, d.name as disposition_name, d.color_hex as color,
...
-- 4. rpc_report_campaign_performance
DROP FUNCTION IF EXISTS rpc_report_campaign_performance;
...
BEGIN
    WITH camp_stats AS (
        SELECT cmp.id as campaign_id,
               ...
               COUNT(DISTINCT c.contact_id) FILTER (WHERE (c.duration > 45 OR EXISTS (SELECT 1 FROM dispositions d WHERE d.id = c.disposition_id AND (d.name ILIKE 'dnc' OR d.name ILIKE 'do not call')))) as contacted,

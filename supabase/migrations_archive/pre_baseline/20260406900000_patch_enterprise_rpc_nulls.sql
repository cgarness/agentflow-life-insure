-- =============================================================
-- Migration: Patch Enterprise Waterfall Queue NULL Handling
-- Date: 2026-04-06
-- Purpose: Resolve critical bug where leads with NULL call_attempts,
--          NULL status, or NULL states were being dropped by the
--          strict WHERE clause comparisons.
-- =============================================================
...
BEGIN
  ...
  eligible_leads AS (
    ...
    WHERE cl.campaign_id = p_campaign_id
      ...
      -- Fix 1: Ensure leads with NULL status aren't dropped by NOT IN
  ...
  WHERE 
    ...
    -- Fix 2: Ensure NULL attempts are treated as 0 and NULL max_attempts as unlimited
    ...
    AND (
      ...
      -- Fix 3: Capture leads with NULL status as 'Queued'
    ...
    AND (
      ...
      -- lead_tz already defaults to 'America/New_York' in the CTE for NULL states

...
Deno.serve(async (req) => {
  ...
  if (!error) return json({ saved: true });
  ...
  const current = (pref?.preference_value && typeof pref.preference_value === "object" && !Array.isArray(pref.preference_value))
    ? pref.preference_value as Record<string, unknown>
    : {};

  const { error: prefError } = await authClient
    .from("user_preferences")
    .upsert(
      {
        user_id: user.id,
        preference_key: "google_calendar_integration",
        preference_value: { ...current, calendar_id: calendarId, sync_mode: syncMode },
  ...
  return json({ saved: true, warning: "using_user_preferences_fallback" });
});

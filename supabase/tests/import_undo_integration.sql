-- =====================================================================================================
-- Import Undo — SQL integration tests (Contacts Build 3, CP2)
-- =====================================================================================================
-- STATUS: PENDING-EXECUTION. Run ONLY against a LOCAL Supabase stack (`supabase db reset` / psql) or an
-- APPROVED Supabase dev BRANCH — never production. Requires migration 20260620000100 applied. The whole
-- script runs in one transaction and ROLLBACKs at the end, so it persists nothing even if misdirected.
--
-- Auth is simulated by setting request.jwt.claims (so auth.uid()/get_org_id() resolve) plus a seeded
-- profile row (get_org_id() profiles fallback). Assertions raise EXCEPTION on failure.
--
-- Covers test-matrix items: eligible undo (atomic delete of leads + tagged campaign rows), import_history
-- marked undone (row preserved), campaign cleanup limited to the exact tag, hidden cross-user engagement
-- blocks undo, transaction integrity (blocked undo deletes nothing), repeated-undo rejection, cross-org
-- rejection, legacy/expired ineligibility, and finalize status mapping.
-- =====================================================================================================

BEGIN;

-- ---- Fixtures -------------------------------------------------------------------------------------
-- Two orgs, an importer (Agent) in org A, an Admin in org A, and a user in org B.
\set ORG_A   '''aaaaaaaa-0000-0000-0000-000000000001'''
\set ORG_B   '''bbbbbbbb-0000-0000-0000-000000000001'''
\set IMPORTER '''aaaaaaaa-0000-0000-0000-0000000000a1'''
\set OTHERUSR '''bbbbbbbb-0000-0000-0000-0000000000b1'''

INSERT INTO public.organizations (id, name) VALUES
  (:ORG_A::uuid, 'Test Org A'),
  (:ORG_B::uuid, 'Test Org B')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.profiles (id, organization_id, role, first_name, last_name, status)
VALUES
  (:IMPORTER::uuid, :ORG_A::uuid, 'Agent', 'Imp', 'Orter', 'Active'),
  (:OTHERUSR::uuid, :ORG_B::uuid, 'Agent', 'Other', 'User', 'Active')
ON CONFLICT (id) DO NOTHING;

-- Helper: act as a given user/org by setting the JWT claims GUC for the txn.
...
BEGIN
  ...
  IF (v_prev->>'eligible')::boolean IS NOT TRUE THEN
    RAISE EXCEPTION 'S1 expected eligible, got %', v_prev;
  END IF;
  ...
  -- audit row preserved + marked undone
  ...
  END IF;
  -- repeated undo is idempotently rejected
  v_undo := public.undo_contact_import(v_hist);
...
BEGIN
  ...
  -- A call owned by a DIFFERENT agent on the imported lead (the case INVOKER could not see).
...
BEGIN
  ...
  -- Org B user attempts to preview/undo Org A's import.
...
-- Personal campaign; l1 assigned to importer (eligible), l2 unassigned (rule-skip). add_leads_to_campaign
-- writes attempted=2/added=1/skipped=1; finalize derives completed_with_skips from DB rows only.
DO $$
DECLARE
  v_org uuid := 'aaaaaaaa-0000-0000-0000-000000000001';
  v_imp uuid := 'aaaaaaaa-0000-0000-0000-0000000000a1';
...
BEGIN
  ...
  v_fin := public.finalize_contact_import(v_hist);   -- idempotent
...
BEGIN
  ...
  v_res := public.add_leads_to_campaign(v_camp, ARRAY[v_l1]);  -- 2-arg form via default
...
DECLARE
  ...
  v_hist uuid := gen_random_uuid(); v_histb uuid := gen_random_uuid();
  v_raised boolean;
BEGIN
  PERFORM pg_temp.act_as(v_imp, v_org);
  INSERT INTO public.campaigns (id, name, type, status, organization_id, user_id)
  ...
  -- S8: campaign mismatch (history.campaign_id = v_camp; call with v_camp2) => reject
  v_raised := false;
  ...
  -- S10: lead id outside the import set => reject
  v_raised := false;
  ...
  -- S11: existing (untagged) membership is skipped and NOT retagged; the new lead is tagged.
  INSERT INTO public.campaign_leads (campaign_id, lead_id, organization_id, status, import_history_id)
    VALUES (v_camp, v_l1, v_org, 'Queued', NULL);  -- pre-existing foreign membership
  PERFORM public.add_leads_to_campaign(v_camp, ARRAY[v_l1, v_l2], v_hist);
  ...
  -- S9: cross-org history (org B) called by org A importer => reject
  INSERT INTO public.import_history (id, file_name, total_records, imported, agent_id, organization_id, campaign_id, imported_lead_ids, import_completion_status)
...
BEGIN
  ...
  -- Only the first "batch" ran (attempted=2 < imported_count=3) — models an interrupted attach.
  PERFORM public.add_leads_to_campaign(v_camp, ARRAY[v_l1, v_l2], v_hist);
  v_fin := public.finalize_contact_import(v_hist);

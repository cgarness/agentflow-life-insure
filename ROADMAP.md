...
- **Recent update (2026-04-23):** **Phone Numbers tab** — Top-level **Phone Numbers** tab nests **Phone number purchase** (`NumberManagementSection`) and **Number reputation** (`NumberReputation`). **Phone & Numbers** keeps Twilio (super-admin), Trust Hub, inline inbound/voicemail, and local presence only. **`usePhoneSettingsController`** runs once in **`PhoneSystem`** so inventory stays shared. Legacy **`?section=number-reputation`** opens **Phone Numbers → Number reputation**. *Files:* **`PhoneSystem.tsx`**, **`PhoneSettings.tsx`**, **`PhoneNumbersPanel.tsx`**, **`SettingsRenderer.tsx`**, **`usePhoneSettingsController.ts`**.

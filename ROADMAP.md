...
- **Recent update (2026-04-22):** **Floating Dialer** top field accepts **name or phone** (digits sync to the manual dial buffer and keypad). After **10 digits**, a **single** `leads` row whose phone matches (NANP last-10) auto-fills the contact card and **replaces the field text with the lead’s name**; multi-match still shows the dropdown. **Hotfix:** `minimized` before `handlePointerMove` (TDZ). Panel **`height: min(max-content, min(600px, 100vh − 5rem))`** trims empty chrome.

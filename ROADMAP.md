...
- **Recent update (2026-04-22):** **Floating Dialer** idle layout is more compact (header, tabs, “Calling From,” search, selected-contact row, keypad) and the panel height follows the viewport (**`min(600px, 100vh − 5rem)`**) so the full keypad and the manual **Call** button (10+ digits) usually fit **without scrolling**. Drag positioning uses the live panel size so the taller panel stays on-screen. **Hotfix:** `minimized` state must be declared **before** `handlePointerMove` (same render) — otherwise production throws **“Cannot access … before initialization”** (minified as e.g. **`Q`**).

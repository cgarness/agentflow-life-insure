...
- **2026-04-09 | [DONE] Critical Fix — Bidirectional Audio (SIP URI Mismatch + Auto-Answer + Webhook Guards)**
  *Files Modified:* `supabase/functions/telnyx-token/index.ts`, `supabase/functions/telnyx-webhook/index.ts`, `src/contexts/TelnyxContext.tsx`, `ROADMAP.md`
  *Edge Functions to Deploy:* `telnyx-token`, `telnyx-webhook`
  *Developer Note:* **ROOT CAUSE** of no-audio-on-either-side: `telnyx-token` created Telnyx Telephony Credentials and generated WebRTC tokens, but never saved the Telnyx-assigned `sip_username` (e.g., `gencredXXXXX`) back to `profiles.sip_username`. The webhook's `telnyxTransfer()` read `profiles.sip_username` to build the SIP URI (`sip:{username}@sip.telnyx.com`) — but this contained the friendly credential **name** (e.g., `agent_abc12345`), not the real Telnyx SIP identity. Per Telnyx docs, only the auto-generated `sip_username` is routable; the credential `name` is metadata. The transfer silently failed, no bridge was established, and neither party heard audio.
  **Fix 1 — telnyx-token (root cause):** Credential lookup now always uses a stable derived name (`agent_{user_id_prefix}`) for the Telnyx API search. After finding or creating the credential, the Telnyx-assigned `sip_username` is extracted from the API response and saved to `profiles.sip_username`. Both SIP credential fallback paths also update the profile. This ensures `telnyx-webhook`'s `telnyxTransfer()` always targets the correct registered SIP address.
  **Fix 2 — TelnyxContext auto-answer:** `call.answer()` now receives `{ localStream: stream }` explicitly (in addition to setting `call.options.localStream`) so the SDK uses the agent's mic regardless of its internal code path. Added comprehensive audio diagnostics logging on `active` state (remote/local track counts, states, enabled flags). Added `remoteAudioRef.play()` safety call if the element is paused.
  **Fix 3 — telnyx-webhook guards:** `handleCallAnswered` now uses the DB update result to determine if the call_control_id belongs to our original outbound leg. Transfer legs (which don't have a matching `calls` row) are detected and skipped, preventing a wasted double-bridge attempt. `handleCallInitiated` no longer creates orphan `calls` records for outbound transfer legs (only inserts for inbound calls without client_state).

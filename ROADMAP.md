...
- **2026-04-23 | [DONE] | Call Recording — dialer respects toggle + retention purge**
  ...
  *Ops (2026-04-23 applied):* Edge secret **`RECORDING_RETENTION_CRON_SECRET`** is set on **`jncvvsvckxhqgqvkppmj`**, **`recording-retention-purge`** is deployed, and migrations are pushed (including **`calls_expired_recording_batch`** + pg_cron). Hosted Supabase **denies** **`ALTER DATABASE ... SET app.settings.*`** for the cron header (**42501**). Migration **`20260423140000_recording_retention_cron_secret_private_table.sql`** adds **`private.recording_retention_cron_secret`** (singleton `id = 1`) and rewires pg_cron to read **`x-cron-secret`** from that row; set the secret once with **`UPDATE private.recording_retention_cron_secret SET secret = '<hex>' WHERE id = 1;`** in the SQL Editor (must match the Edge secret).

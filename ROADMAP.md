...
- **2026-05-01 | [DONE] | Full view — remove duplicate From in conversation header; email bubble width**
  *What:* Conversation card header no longer repeats **From** (picker stays on **top toolbar** for SMS outbound numbers). Email rows use **`max-w-[85%]`** strips, **`rounded-2xl`** + directional **`rounded-tr-sm`/`rounded-tl-sm`**, subject/chevron accordion. *Follow-up:* **Sent** emails use **`#007AFF`** bubble + white subject line; **Received** use gray peer bubble (**`#E9E9EB`** / **`#262629`**). **Purple Mail** icon sits **inside** the collapsed row (dialer pattern); full body only after chevron expand. *File:* **`FullScreenContactView.tsx`**.

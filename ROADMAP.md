...
- **2026-05-01 | [DONE] | Full view — remove duplicate From in conversation header; email bubble width**
  *What:* Conversation card header no longer repeats **From** (picker stays on **top toolbar** for SMS outbound numbers). Email rows use **`max-w-[85%]`** strips with violet side icon (**hover opacity** matches calls/SMS), **`rounded-2xl`** + directional **`rounded-tr-sm`/`rounded-tl-sm`**, subject/chevron accordion. *File:* **`FullScreenContactView.tsx`**.

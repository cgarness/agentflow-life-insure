...
- **2026-05-01 | [DONE] | BUGFIX — Contact full view center column: compose tabs + first-pass borders**
  *What:* **`MessageComposePanel`** SMS/EMAIL uses **segmented control** chrome (**`bg-muted`** track, **`bg-card`** active); dialer inherits via shared panel. **Center column** first pass used **`border-border`** edges (later strengthened — see next log). *Files:* **`MessageComposePanel.tsx`**, **`FullScreenContactView.tsx`**.

- **2026-05-01 | [DONE] | Contact full view — visible center column seams + email rows (subject + expand)**
  *What:* **Column chrome:** three-column body row uses **`border-t`**; **center** owns both vertical rails (**`border-l border-r`** on **`border-neutral-300/90`** / dark **`border-neutral-600`**) so dividers are not washed out; **left** and **right** drops outer edge borders that previously **doubled** the center line. **Email:** restored **violet mail badge** card; collapsed row shows **Sent/Received**, **subject only**, **chevron**; expand reveals full body (quoted lines stay muted). *File:* **`FullScreenContactView.tsx`**.

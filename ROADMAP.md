...
- **⚠️ DOC DRIFT (2026-04-29) — Email tables documented as DONE were not in production**: ROADMAP work-log entry from `2026-04-29 [DONE]` (Codex) declared migrations `20260429143000_email_inbox_connections_and_contact_emails.sql` and `20260429152000_email_oauth_states.sql` complete, but `user_email_connections`, `email_sync_cursors`, `contact_emails`, and `email_oauth_states` did not exist in production at the start of this audit deploy (2026-04-29 22:48 UTC). Both migrations were applied during this deploy via Supabase MCP `apply_migration` immediately before the cross-org RLS tests. **Lesson:** an entry being marked `[DONE]` in ROADMAP does not guarantee `db push` ran against production — the next coding agent should verify with `information_schema.tables` before composing dependent work.
...
- **2026-04-29 | [DEPLOYED] | Email module compliance audit + shared Google token helper**
  *Deploy status (2026-04-29):* All five email Edge Functions (`email-connect-start` v1, `email-connect-callback` v1, `email-disconnect` v1, `email-send-contact-message` v1, `email-sync-incremental` v1) and the canary `google-calendar-inbound-sync` v463 deployed to `jncvvsvckxhqgqvkppmj` with `verify_jwt: false`. Audit migration recorded as `20260430024520_20260429190000_email_module_audit`. Prerequisite migrations `20260429143000_email_inbox_connections_and_contact_emails` and `20260429152000_email_oauth_states` were applied during this pass (they had been declared `[DONE]` earlier but were never landed in production — see DOC DRIFT blocker in Section 1). Cross-org RLS verified: positive control returned 3 profiles in own org with valid spoof; leak tests on `user_email_connections`, `contact_emails`, `email_sync_cursors`, `email_oauth_states` all returned 0; bonus active-RLS proof — populated `profiles` table returned 0 rows when querying org_b from an org_a-spoofed session. `email-sync-incremental` cron lockdown verified: HTTP 401 with no `x-cron-secret` and HTTP 401 with wrong secret. **Awaiting cross-org test pass + canary cron tick before merge to main.** Note: canary cron tick was satisfied by manual `pg_net` invocation (HTTP 200, clean no-work response) — automatic cron tick is unavailable because the schedule itself is missing in production (see Section 1 BLOCKER).

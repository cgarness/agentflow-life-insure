...
- **Recent update (2026-04-24):** **Settings → Master Admin** — Visible in the settings sidebar and routable via **`?section=master-admin`** only for **`useOrganization().isSuperAdmin`** (JWT **`is_super_admin`** or **`profiles.is_super_admin`**). Others are redirected to **My Profile**. Replaces the prior hardcoded email/UID check. *Files:* **`SettingsPage.tsx`**, **`SettingsRenderer.tsx`**, **`Sidebar.tsx`**.
- **Recent update (2026-04-24):** **`custom_fields` tenant + per-user isolation** — Migration **`20260424120000_custom_fields_created_by_and_rls.sql`**: adds **`created_by`** (FK → **`profiles`**); replaces permissive SELECT (**`organization_id IS NULL`** leaked rows to every org) with policies using **`get_org_id()`**, **`get_user_role()`**, and **`is_super_admin()`**. **Agents** see org-wide legacy rows (**`created_by` NULL**) plus **their own** definitions; **Admin / Team Leader / Team Lead** see all fields in the org. **Any org member** may insert a personal field (**`created_by = auth.uid()`**); admins may insert **org-wide** (**`created_by` NULL**). Client: **`customFieldsSupabaseApi.getAll(organizationId)`** scopes queries; **`create`** sets **`created_by`** from the session (optional **`orgWide`** for admins). Settings **Field Layout** + CSV import use the same API. **Apply:** **`npx supabase db push`** (or SQL Editor). **Vitest:** **`src/lib/custom-fields-settings.test.ts`**.
...
- **Recent update (2026-04-24):** **`create-user`** confirmation email HTML refreshed to match **AgentFlow** invite styling (gradient accent bar, badge, hero typography, monospace fallback link box, dark radial backdrop). Subject: **You're almost in — confirm your AgentFlow email**. **Deploy:** **`supabase functions deploy create-user`**.
- **Recent update (2026-04-24):** **Post-signup onboarding wizard** at **`/onboarding`** (3 steps: profile + resident state, NPN/licensed states/timezone/commission, then founder agency name/timezone/team-size **or** invited read-only agency card). **Gate:** `user.user_metadata.needs_app_wizard` and not `app_wizard_completed`, with **confirmed email** — **`ProtectedRoute`** redirects here; **`login`**, **`AuthCallback`**, and **`PublicRoute`** use **`resolvePostAuthPath`**. Completing the wizard sets **`profiles.onboarding_complete`**, updates org name for founders (best-effort), and sets **`app_wizard_completed`** (+ optional **`team_size_intent`**) in **Auth user metadata**. **`create-user`** / **`accept-invite`** set **`needs_app_wizard`** + **`signup_source`**. **Deploy:** **`supabase functions deploy create-user`** and **`supabase functions deploy accept-invite`** after merge.
- **Recent update (2026-04-24):** Onboarding **step 2** — all fields **optional**; **commission level** is a **manual numeric-only** input (digits stored as entered, e.g. `105`). **Finish** calls **`refreshSession`** in a short loop until JWT **`app_metadata`** includes **`role`** and **`organization_id`** so **`profiles` UPDATE** RLS (`get_user_role()` from JWT) succeeds; setup errors surface the **PostgREST / auth message** in the toast.
- **Recent update (2026-04-24):** **Profile after onboarding** — `fetchProfile` used a **10-column “safe” fallback** when any `select('*')` error contained “does not exist”, then **`auth.updateUser`** (wizard finish) refetched and **replaced** React `profile` with that subset so **Settings → My Profile** looked like only name + email. **Fix:** fallback now uses **`PROFILE_FETCH_FALLBACK_SELECT`** (`src/lib/profile-fetch-columns.ts`: phone, resident state, timezone, NPN, licensed states, commission, org, onboarding flags, goals, etc.); **`updateProfile`** uses **`.update(...).select().maybeSingle()`** so context matches the row Postgres returned.
...
- **Recent update (2026-04-23):** **Purchase default friendly name** — **`NumberManagementSection`** passes Twilio search row **`locality` + `region`** as optional **`friendly_name`** on **`twilio-buy-number`**; Edge function prefers it over Twilio’s default when inserting **`phone_numbers.friendly_name`** (still editable in the UI). **Deploy:** **`supabase functions deploy twilio-buy-number`** to production after merge.
- **Recent update (2026-04-23):** **Purchase modal — cart** — Search results show **$3.00 (estimate)** per row and **Add to cart**; cart strip with **View cart** (line items + remove), **Clear cart**, **Purchase all** (sequential **`twilio-buy-number`** calls). Removed long header/footer copy from the dialog. *File:* **`NumberManagementSection.tsx`**.
...
- **Schema:** `organizations.twilio_subaccount_sid` (unique nullable), `twilio_subaccount_auth_token_vault_key`, `twilio_subaccount_status` (CHECK: `pending` / `active` / `pending_manual` / `suspended` / `closed`, default `pending`), `twilio_provisioned_at`. New table `public.provisioning_errors` (id, organization_id, attempt_number 1–10, error_code, error_message, twilio_response JSONB, created_at) + index on `(organization_id, created_at DESC)`.
- **Trigger config:** `private.twilio_provisioning_config` singleton (`id=1`) holds `supabase_url` + `service_role_key`. Populate **once via SQL Editor** (not migration) — same pattern as `private.recording_retention_cron_secret`. Until populated, the trigger logs a `RAISE WARNING` and the new org row stays in `pending` (Edge Function never fires).
- **Vault writer:** `public.set_twilio_subaccount_token(p_org_id, p_token)` — `SECURITY DEFINER`, `EXECUTE` granted to **`service_role` only**. Uses `vault.create_secret` / `vault.update_secret` so the Edge Function never touches the `vault` schema directly.
- **State (Phase 2 + HOTFIX, 2026-05-04 / 2026-05-05):** **`twilio-token`** Edge Function mints Voice JWTs with status-gating per org subaccount. Flow: validates Bearer JWT → resolves `profiles.organization_id` → loads `organizations.twilio_subaccount_sid / _vault_key / _status` → status-gates (`pending` → 503 `PROVISIONING_PENDING`, `pending_manual` → 503 `PROVISIONING_FAILED`, `suspended`/`closed` → 403 `TELEPHONY_SUSPENDED`) → calls RPC `public.get_twilio_subaccount_token(uuid)` to verify Vault credentials → signs HS256 JWT with `iss = TWILIO_API_KEY_SID` (master), **`sub = TWILIO_MASTER_ACCOUNT_SID`** (master — see HOTFIX note below), `grants.voice.outgoing.application_sid = TWILIO_TWIML_APP_SID`. Response shape unchanged: `{ token, identity, expires_in: 14400 }`. **⚠️ HOTFIX (2026-05-05):** Phase 2 originally set `sub = subaccount_sid`, causing **ConnectionError 53000** across all orgs because TwiML App `AP6ac23752609fdee79751693a2a223cd8` lives on the master account — a JWT scoped to a subaccount cannot reference a TwiML App on the master account. Fixed in `twilio-token` **v15**: `sub` is now `TWILIO_MASTER_ACCOUNT_SID`. Subaccount SID is still fetched and validated for status-gating; it is not used in the JWT `sub` claim.
- **Vault reader:** `public.get_twilio_subaccount_token(p_org_id uuid) RETURNS text` — `SECURITY DEFINER`, `search_path = public, vault, pg_temp`, `EXECUTE` granted to **`service_role` only** (REVOKE from `anon`/`authenticated`). Reads `vault.decrypted_secrets.decrypted_secret` matching `twilio_subaccount_token_<org_id>`; returns NULL when missing.
- **State (Phase 3, 2026-05-04):** Number purchase / search and CNAM (Trust Hub) Edge Functions now call Twilio under each org's **subaccount SID + Vault auth token** instead of master credentials. New shared module **`supabase/functions/_shared/twilioSubaccountCreds.ts`** (`loadSubaccountCreds`) centralises status-gating + Vault read; consumed by **`twilio-buy-number`**, **`twilio-search-numbers`**, **`twilio-trust-hub`**. New Edge Function **`retry-twilio-provisioning`** (Super Admin only — JWT claim `is_super_admin === true` AND `profiles.is_super_admin = true`) accepts `{organization_id}`, idempotent (returns `already_provisioned` when SID exists), restricted to `pending` / `pending_manual` orgs, delegates to `provision-twilio-subaccount` via internal fetch with service-role bearer. Super Admin UI (`SuperAdminDashboard`) gains a **Twilio Provisioning** panel listing every org with a status badge + Retry button (visible only when retryable).

...
- **Recent update (2026-04-22):** **Floating Dialer** idle layout is more compact (header, tabs, “Calling From,” search, selected-contact row, keypad) and the panel uses **`height: min(max-content, min(600px, 100vh − 5rem))`** so short views **shrink to content** (no tall empty box) while long views still cap and scroll inside the body. Drag positioning uses the live panel size. **Hotfix:** `minimized` state must be declared **before** `handlePointerMove` (same render) — otherwise production throws **“Cannot access … before initialization”** (minified as e.g. **`Q`**).

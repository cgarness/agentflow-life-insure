...
### 📡 Multi-Tenant Twilio Provisioning `[ACTIVE]`
...
- **Out of scope (deferred):** Phase 3 covers number purchase under each subaccount, per-subaccount TwiML App provisioning question, and CNAM. Client (`DialerPage.tsx`, `TwilioContext.tsx`) is untouched in Phase 2 — `PROVISIONING_PENDING` / `PROVISIONING_FAILED` / `TELEPHONY_SUSPENDED` codes surface as generic Twilio init errors until UX polish in a later BUILD.
- **Next Up:** Phase 3 — number purchase under each subaccount, CNAM registration, and decide whether the master TwiML App is reused or per-subaccount TwiML Apps are provisioned at subaccount creation time.

...
**Shipped (2026-06-29).** PR [#333](https://github.com/cgarness/agentflow-life-insure/pull/333) **MERGED** to `main` — merge commit `67a983211b91d2d510d56e9a82bb2dac73ac0f94` (mergedAt 2026-06-29T19:21:35Z; `origin/main` HEAD). **Supabase untouched** (frontend-only — no migration/RPC/RLS/service-role). Vercel production auto-deploys from `main` — deploy status to confirm in the Vercel dashboard. *(Docs correction recorded 2026-06-29 from branch `claude/contacts-unassigned-visibility-harden`: this entry previously read "PR open, awaiting review/deploy".)*

**Blockers / next steps.** Chris visual smoke on production (filter drawer slide-in + scroll; selected-state strength in light/dark; Agents `CA` chips; Import History row → drawer with loading/empty/undone states; Leads scope pills in the tab row; single initial load with no flicker; full-card Kanban drag with the card following the pointer + convert-stage still opens ConvertLeadModal). The agent has no prod CRM login, so visual smoke is human-run.
...
**Merged + deployed.** PR [#325](https://github.com/cgarness/agentflow-life-insure/pull/325) (feature commit `a60062a0369c2c2983dbaf0fcd9fd94779c89fa0`) → merged to `main` via merge commit **`8340e2f9dfde149c4e4e6dc32fc421a46d8324fa`** (an ancestor of current `main`; `main` later advanced via the ai-testing PRs #329/#330). **Vercel production deploy `dpl_4aWSUaxA2c7kf4ZSteVCWNp8Kctm` → READY** (project `agentflow-life-insure`, target production); production aliases **`agentflow-life-insure.vercel.app`** + **`www.fflagent.com`** both return **HTTP 200**. **Supabase untouched** (no migration, no edge function, no MCP call). Frontend-only — see scope below.
...
- **Edge Functions DEPLOYED to prod** via linked Supabase CLI: `ai-testing-start-browser-session` (new) and `ai-testing-end-call` (updated, browser sessions → `completed`). Both `verify_jwt=false` (in-code Super Admin auth); live endpoint returns **401** unauthenticated. Security advisors: **no `ai_test_sessions` findings**.
- **Render `ai-voice-bridge` LIVE** (`srv-d8flo7rtqb8s73f3jro0`, deploy `dep-d8unhv4vikkc739f6ne0`, commit `e1e1699`, status `live`). Verified `GET https://ai-voice-bridge-ouez.onrender.com/ready` → `paths` now include **`/browser/deepgram`** + **`/browser/inworld`**; `configured` = openai/deepgram/inworld/supabase all `true`.
- **Vercel frontend** auto-deploys from `main` on the merge commit (2-column Test UI). PR checks `Vercel – agentflow` + `Vercel – agentflow-life-insure` were **SUCCESS** on the PR; the only non-green check was the known non-required `Supabase Preview` (full-history branch-replay debt).
...
**Remaining to fully ship.** Only (1) Human Super Admin verification in-browser (mic → greeting → converse on Deepgram + Inworld → Stop; phone fallback), once the Vercel deploy of the merge commit finishes. Docs: `docs/AI_TESTING_SETUP.md` section 8e. All backend pieces (Supabase migration + edge functions + Render bridge `/browser/*` routes) are live in prod.
...
**Production migration live** (applied at CP3C): `contacts_permissions_enforcement` as MCP version **`20260625162731`** (on-disk `supabase/migrations/20260624120000_contacts_permissions_enforcement.sql`, SHA-256 `a77168c3…`). This frontend (un-deployed until now) is what exercises the new RPCs/policies/scope.

**Net shipped (Build 5).** A configurable, per-org Contacts permissions framework on the existing `role_permissions` table: a stable-key catalog (`CONTACTS_PERMISSIONS`, 25 keys) + a Settings → Permissions Contacts module (danger copy + System Rules panel), `usePermissions().hasContactsPermission`, and frontend gating for delete/import (+ `/contacts/import` route)/assign/bulk-assign/bulk-status/add-to-campaign/edit/inline-status/Kanban-drag + a permission-gated `unassigned`/`agency`(view_all) scope model. **Backend enforcement:** `has_contacts_permission` + `_contacts_permission_default` (owner-only) + `delete_contact` (SECURITY DEFINER; permission + same-org + owner/downline/admin; hard-delete parity incl. server-side `campaign_leads` cleanup; telemetry preserved) + additive SELECT-only RLS policies `leads_select_unassigned_pool` / `leads_select_view_all_pool` + an additive `unassigned` branch in `_contacts_filtered_leads`. **Lead → Client conversion stays universal/ungated — no key, never in Settings, RPC unchanged.**

**Post-deploy verification.** App shell HTTP 200 on both prod aliases; the production deployment's commit equals the merge commit; no Vercel build errors. Backend read-only (post-deploy): the 3 functions + 2 policies live; **leads 517 / clients 0 / recruits 0 / wins 0 unchanged**. CP3C read-only behavior verification (still valid): permission defaults correct for Agent/TL/Admin/Super-home/cross-org; default **Agent sees 0 unassigned**; **Admin sees 517 + 507 via the `unassigned` scope**; new policies SELECT-only (no UPDATE/DELETE broadening). **Smoke limitation:** the agent has no production CRM login, so the interactive **logged-in** UI click-through (Settings → Permissions Contacts module save, an Agent losing/gaining delete/import, the Unassigned scope for a permitted user) is handed to Chris; the agent verified app-shell + deployment + read-only backend only. **`delete_contact` was NOT invoked on production** (destructive) — verified by introspection + on the CP3B branch.

**Advisor posture (unchanged from CP3C):** no new ERROR (2 pre-existing `app_config`/`webhook_debug_log`); migration-attributable = 2 intentional DEFINER-executable WARNs (`has_contacts_permission` + `delete_contact`) + 1 `multiple_permissive_policies` WARN on leads SELECT (the additive-policy trade-off).

**Deferred (unchanged):** Add-to-Campaign backend parity (Campaigns permissions module) · dedicated import RPC (import closeout) · least-privilege hardening migration (lead-source anon-revoke + `pg_temp`; `get_user_role`/`is_super_admin` search_path) tracked as separate SECURITY work · `view_all` UI now backend-enforced (no longer inert). **No Twilio/Dialer, queue claim/advance, import-undo, or conversion changes in Build 5. Build 6 (UI Closeout + Refactor) not started.**

---

2026-06-25 | [CHECKPOINT 3C — backend enforcement APPLIED to production; frontend not deployed; not shipped] Contacts Build 5 — Permissions Framework + Contacts Permission Wiring

**Migration applied to production** (`jncvvsvckxhqgqvkppmj`) via Supabase MCP `apply_migration`, name `contacts_permissions_enforcement`, result `{success:true}`. **Recorded MCP version `20260625162731`**; on-disk file `supabase/migrations/20260624120000_contacts_permissions_enforcement.sql`, **SHA-256 `a77168c3a66b888d0bf3d73eb96ca2fb75dfe8a4e07ea582529702ca1afedccd`** (matched the CP3B-validated, Chris-approved file). Branch `claude/contacts-build5-permissions-cp2`. Scope = the 3 functions + 2 additive SELECT policies + the `_contacts_filtered_leads` `unassigned` branch only. **No production data mutated (schema only); no deploy; nothing committed/pushed/PR'd/merged. Build 6 not started.**

**Pre-apply guard (all passed):** SHA exact; migration not previously recorded; functions/policies absent; current `_contacts_filtered_leads` had no `unassigned` branch; content scope clean (no Twilio/queue/conversion/import-undo/`campaign_leads`-policy/mutation-policy/`anon`-grant — the only `campaign_leads` reference is the approved DELETE inside `delete_contact`).

**Live functions/ACLs (prod):** `_contacts_permission_default` = INVOKER/IMMUTABLE/`search_path=public,pg_temp`/**owner-only** (anon✗ auth✗ svc✗); `has_contacts_permission` = **SECURITY DEFINER/STABLE**/safe search_path/**anon✗ auth✓ svc✓**; `delete_contact` = **SECURITY DEFINER**/safe search_path/**anon✗ auth✓ svc✓**. Policies: `leads` 1→**3** SELECT policies (`leads_select_unassigned_pool` + `leads_select_view_all_pool`, both PERMISSIVE/SELECT-only/authenticated/org-scoped via `get_org_id()` + `has_contacts_permission`). `_contacts_filtered_leads` `unassigned` branch live. **No data change:** leads 517 / clients 0 / recruits 0 unchanged.

**Read-only prod behavior (real profiles, GUC simulated, reset after):** permission defaults all correct (Agent view_assigned✓/delete✗/view_unassigned✗/view_all✗/unknown✗; TL view_unassigned✓/import✓/delete✗; Admin✓; Super-Admin home-org✓; cross-org✗). Leads visibility: default **Agent → 0 unassigned visible / 0 via `unassigned` scope** (hidden); **Admin → 517 total, 517 `agency` scope (unchanged), 507 via `unassigned` scope** (the on-path proven on real prod data). New policies are SELECT-only → no UPDATE/DELETE broadening. `delete_contact` verified by introspection only — **not invoked on prod** (no prod records created/deleted).

**Advisor delta — migration-attributable:** Security = 2 intentional `authenticated_security_definer_function_executable` WARNs (`has_contacts_permission` + `delete_contact`; mirrors prod `convert_lead_to_client_atomic`); **no new ERROR** (2 pre-existing `app_config`/`webhook_debug_log` unchanged). Performance = 1 expected `multiple_permissive_policies` WARN on leads SELECT; **0 ERROR**. My functions absent from `function_search_path_mutable`/`anon_security_definer`/`auth_rls_initplan`.

**Generated types + casts.** `src/integrations/supabase/types.ts` regenerated from prod — **+9 lines, 0 removed** (the 3 new RPCs; no drift). Removed the narrow `(supabase as any)` casts from the 3 `delete()` methods → typed `supabase.rpc("delete_contact", …)`.

**Repo:** `tsc` clean · `vitest` **331/331** · targeted ESLint **0 errors** (benign pre-existing warnings) · `git diff --check` clean.

**HOLD for CP4.** Migration live in prod; **frontend NOT deployed** (new RPC + view_unassigned/view_all/unassigned-scope reachable only from the un-deployed Build 5 frontend; existing paths unaffected — additive + default-off preserves current behavior); nothing committed/pushed/PR'd/merged/deployed; Build 5 not shipped; Build 6 not started. Next: CP4 commit Build-5 files → PR → CP5 merge/deploy/smoke + shipped WORK_LOG entry.

---

2026-06-25 | [CHECKPOINT 3B — backend enforcement implemented/validated on non-production branch; NOT applied to production; not shipped] Contacts Build 5 — Permissions Framework + Contacts Permission Wiring

**Backend enforcement for the CP2 permissions framework, validated on a temporary Supabase dev branch only.** Branch `claude/contacts-build5-permissions-cp2`. Approvals consumed: `#APPROVE_RLS_CHANGE` (additive read-only `view_unassigned` + `view_all` SELECT policies only) + `#APPROVE_SUPABASE_BRANCH_COST` (one branch @ **$0.01344/hr** = prior range, created→validated→**deleted**). Per Chris's CP3A change, **`view_all` is implemented now (not deferred)**. **No production apply, no production data mutation, no deploy, nothing committed/pushed/PR'd/merged. Build 6 not started.**

**Migration (FILE only, NOT applied to prod):** `supabase/migrations/20260624120000_contacts_permissions_enforcement.sql`, SHA-256 **`a77168c3a66b888d0bf3d73eb96ca2fb75dfe8a4e07ea582529702ca1afedccd`**. Contents: `_contacts_permission_default(text,text)` (owner-only map, **parity-checked 25/25 vs `CONTACTS_PERMISSIONS`**), `has_contacts_permission(text)` (STABLE DEFINER; uid+org server-derived; Admin/SA-home full; stored→default; unknown→false; anon✗/auth✓/svc✓), `delete_contact(text,uuid)` (DEFINER; permission + same-org + owner/downline/admin; **hard-delete parity** incl. server-side `campaign_leads` cleanup for leads; telemetry preserved via SET NULL), additive PERMISSIVE SELECT policies `leads_select_unassigned_pool` + `leads_select_view_all_pool` (SELECT-only, never broaden UPDATE/DELETE, never cross-org; hierarchical ALL policy untouched), and a verbatim `_contacts_filtered_leads` + ONE additive `unassigned` scope branch (Kanban/table contract unchanged). `NOTIFY pgrst`.

**Frontend:** `supabase-contacts/clients/recruits.ts` `delete()` → `delete_contact` RPC (narrow cast until CP3C type regen). Scope (D-scope-model): `ContactScope += "unassigned"`; `useContactScope.computeAvailableScopes` now driven by the catalog keys (mine; team if downline; `unassigned` if view_unassigned; `agency` if view_all) — supersedes the legacy Data-Access pill; `ContactScopeSelector`/`scopeLabel`/`resolveOwnerAgentIds` handle `unassigned` (Leads-only; degrades to self for Clients/Recruits); `Contacts.tsx` hides `unassigned` off non-Leads tabs + resets on tab-away.

**Branch validation — ALL integration tests PASS** (`supabase/tests/contacts_permissions_integration.sql`): T1 has_contacts_permission (anon/Agent-default/TL-default/Admin/Super-Admin-home/cross-org/stored-override/missing-key); T2 delete_contact (deny-without-perm, invalid-type, peer-denied, owner-allowed **with telemetry call surviving lead_id SET NULL**, cross-org-blocked, not-found, TL-downline, Admin); T3 view_unassigned + view_all (off→hidden, on→visible, cross-org hidden, **UPDATE/DELETE NOT broadened**); T4 `unassigned` scope returns the org pool only. **Faithful minimal harness** used (replay debt; real helpers + prod-typed tables + real hierarchical RLS). **Advisors (migration-attributable):** security = 2 intentional DEFINER-executable WARNs (mirrors prod `convert_lead_to_client_atomic`/`undo_contact_import`); performance = 1 expected `multiple_permissive_policies` WARN (additive-policy trade-off); all else harness artifacts / pre-existing. **EXPLAIN:** ~1.4 ms, constant-arg STABLE function (no per-row blow-up). **Fixed during validation:** invalid-hex test UUIDs + the `_contacts_permission_default` anon grant (Supabase default-privileges required an explicit `REVOKE FROM anon, authenticated, service_role`). **Branch deleted; billing stopped** (confirmed).

**Repo:** `tsc` clean · `vitest` **331/331** · targeted ESLint **0 errors** (benign pre-existing warnings only) · `git diff --check` clean.

**HOLD for CP3C approval** (production apply of the migration → ACL/advisor/parity verify → regenerate `types.ts` + drop casts → CP4 PR → CP5 deploy/smoke). Frontend is NOT deployed until the migration is live in prod. **No production mutation occurred in CP3B; Build 5 not shipped; Build 6 not started.**

---

2026-06-24 | [CHECKPOINT 2 — Contacts permission catalog + Settings UI/frontend wiring; no backend mutation; not shipped] Contacts Build 5 — Permissions Framework + Contacts Permission Wiring

**Direction (CP1B, approved):** Build 5 pivoted from "hardcode Contacts role gates" to a **configurable, per-agency permissions framework, Contacts-first, backend-enforceable** — reusing/extending the existing `role_permissions` framework (no rebuild, no custom roles). Branch `claude/contacts-build5-permissions-cp2` (off `origin/main` `9b395f6`). **CP2 = frontend + catalog only. No SQL/migration/RLS/RPC/Supabase mutation; nothing committed/pushed/PR'd/merged/deployed; Build 6 not started.**

**Locked decisions applied:** D-roles (Agent + Team Leader configurable; Admin/Super Admin locked full-access), D-storage (new normalized `permissions.contacts` block; legacy `f` kept for non-Contacts modules), D-unassigned-default (Agent off / Team Leader on / Admin+SA always on), D-scope-model (Contacts catalog supersedes the legacy Data Access pill for Contacts), D-import-enforcement (FE gate + existing INSERT/RLS floor; no import RPC in CP2). **Conversion is hardcoded universal — no catalog key, not rendered in Settings, not gated; conversion RPC/win trigger untouched.**

**Permission catalog (`src/config/permissionDefaults.ts`).** Added `CONTACTS_PERMISSIONS` — 25 stable keys (`contacts.<entity>.<action>`) grouped Leads/Clients/Recruits/Engagement, each with label/help/`danger`/per-role defaults; NO conversion key; NO `contacts.clients.create` (manual client create stays universal). Helpers: `resolveContactsPermissionDefault(role,key)` (Admin/SA→true), `getDefaultContactsPermissions(role)`, `mergeContactsPermissions(role,stored)` (stored-override wins, missing→default, unknown dropped), `resolveContactsPermission(role,isFullAccess,storedBlock,key)` (pure, the exact `hasContactsPermission` logic). `RolePermissions` gained optional `contacts?: Record<string,bool>`.

**Hook (`usePermissions.ts`).** Added `hasContactsPermission(key)` (Admin/SA full → stored override → catalog default; never display-name-keyed; never gates conversion); `normalizePermissions` preserves the `contacts` block; fixed the stale "do not consume this hook" comment.

**Settings → Permissions (`Permissions.tsx`).** New "Contacts Permissions" module (rendered first) from the shared catalog — grouped, with danger chips + warning copy on delete/import/undo, and a read-only "System rules" panel listing the non-configurable boundaries (tenant isolation, no service-role exposure, **conversion available to all legitimate org-scoped users**, telemetry/Twilio safety). Agent/Team Leader configurable; Admin/SA locked full. Persists the normalized `permissions.contacts` block via the existing `(organization_id, role)` upsert + activity-log diff; missing keys fall back to defaults; legacy `p/f/d/c/s` groups unchanged.

**Frontend gating.** `Contacts.tsx`: import button + bulk delete (per-entity) + bulk assign + bulk status + add-to-campaign + row edit/delete + create buttons + inline status selects gated by the catalog (outer "Bulk Actions" feature gate replaced by per-control gating). `FullScreenContactView.tsx`: EDIT + Delete gated per-entity; **Convert left ungated**. Kanban `canDrag` threaded `ContactKanbanBoard → KanbanColumn → KanbanCard` (drag-handle hidden + droppable/sortable disabled + `handleDragEnd` guard) bound to `contacts.leads.update_status`. `PageGuard.tsx` gained an optional `contactsPermission` prop; `App.tsx` `/contacts/import` route now requires `contacts.leads.import` (not reachable by URL when disabled).

**Tests (+26).** `contactsPermissions.test.ts` (catalog integrity — unique keys, defaults, danger flags, **no conversion key**; default resolution incl. Admin/SA full + D-unassigned-default; merge precedence; `resolveContactsPermission` stored-true/false/missing/Agent/TL/full/no-role). `pageGuardContacts.test.tsx` (import route allowed/denied/no-prop). `contactsGatingRender.test.tsx` (Import CSV + Add gated on/off). `permissionsSettingsContacts.test.tsx` (Contacts module renders from catalog, danger copy, conversion-as-rule-not-toggle, save writes `permissions.contacts`). Updated `contactsRender.test.tsx` to mock `usePermissions` (Contacts now consumes it directly).

**Verification.** `npx tsc --noEmit` clean · `npx vitest run` **328/328** · targeted ESLint **0 errors / 8 benign warnings** (pre-existing exhaustive-deps + unused-disable) · `git diff --check` clean. Pre-existing unrelated working-tree files (`seed-test-leads.mjs`, `hypercheap-voice-bridge/*`, `tsconfig*.tsbuildinfo`) untouched.

**HOLD for CP3 approval.** Backend enforcement deferred to CP3 (approval-gated): `has_contacts_permission` + `_contacts_permission_default` helpers, `delete_contact` DEFINER RPC, additive unassigned-pool SELECT policy (`#APPROVE_RLS_CHANGE`), conversion-authorization audit, optional lead-source anon-revoke/search_path hardening. **No migration applied, no Supabase mutation, nothing committed/deployed; Build 5 not shipped; Build 6 not started.**

...
2026-05-22 | [READY — migration not yet applied] Control Center v1. What: New platform-admin-only experience for monitoring AgentFlow itself — Overview / Feature Tracker / Issue Tracker / Health Checks. Lives at `/control-center/*` behind a new `PlatformAdminRoute` guard; renders in its own `ControlCenterLayout` (own sidebar; no CRM TopBar, sidebar, or FloatingDialer). New platform-level role added on `profiles.platform_role` (nullable, CHECK allows `NULL` or `'platform_admin'`). Independent from `is_super_admin` — does NOT change Super Admin behavior. Access gated by a new `public.is_platform_admin()` SQL helper that reads `profiles` directly (no JWT claim, no `custom_access_token_hook` change → no token reissue required). Four new tables (`control_center_features`, `control_center_issues`, `control_center_health_checks`, `control_center_health_check_runs`) with full enum CHECK constraints, indexes, `set_updated_at` triggers, RLS enabled, and platform-admin-only SELECT/INSERT/UPDATE/DELETE policies. v1 Run Checks button is a **stub** — records a run row + sets `last_run_at`, no live probes against Twilio/Supabase/Vercel. Empty states everywhere; zero mock seed rows in production paths. Existing `system_status` table untouched (different scope; future consolidation candidate). Zero changes to dialer / TwilioContext / calls / webhooks / CRM nav.

Notes: Migration file `supabase/migrations/20260522120000_control_center_v1.sql` written but **NOT applied** — held for Chris's `apply migration`. New files: `src/components/auth/PlatformAdminRoute.tsx`, `src/components/control-center/{ControlCenterLayout,ControlCenterSidebar,StatusBadge,SeverityBadge,SummaryCard,EmptyState}.tsx`, `src/components/control-center/features/{FeatureTable,FeatureFormModal}.tsx`, `src/components/control-center/issues/{IssueTable,IssueFormModal}.tsx`, `src/components/control-center/health/{HealthChecksTable,HealthCheckFormModal,RunChecksButton}.tsx`, `src/pages/control-center/{ControlCenterOverviewPage,ControlCenterFeaturesPage,ControlCenterIssuesPage,ControlCenterHealthPage}.tsx`, `src/hooks/{useIsPlatformAdmin,useControlCenterFeatures,useControlCenterIssues,useControlCenterHealthChecks}.ts`, `src/lib/control-center/{constants,types,featureSchema,issueSchema,healthCheckSchema}.ts`. Modified: `src/App.tsx` (new `<Route element={<PlatformAdminRoute><ControlCenterLayout /></PlatformAdminRoute>}>` block; existing CRM routes untouched), `src/contexts/AuthContext.tsx` (added `platform_role: string | null` to `Profile` interface — typing only). Deliberately did NOT add `platform_role` to `PROFILE_FETCH_FALLBACK_SELECT` per its own header comment ("avoid rarely-deployed columns"); the primary `select('*')` path picks it up after migration. `src/integrations/supabase/types.ts` will be regenerated via Supabase MCP **after** Chris approves migration apply. `npx tsc --noEmit` clean. Vitest suite: 56/56 assertions pass; 3 pre-existing module-load failures (`supabaseUrl is required` in test env) are unchanged by this work — verified by re-running on stashed HEAD.

Next: await `apply migration` from Chris → MCP `apply_migration` → regenerate types → set `platform_role='platform_admin'` on Chris's profile → manual browser verification (non-platform user redirected; platform_admin lands on Control Center; CRM still works; RLS denies non-platform reads via SQL spot-check) → commit + push.

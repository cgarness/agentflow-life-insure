...
import {
  ...
  GraduationCap, CheckCircle2, ArrowRight, Save, Clipboard, Play, FileText, MessageSquare
} from "lucide-react";
...
const cdmStatusBadgeColor: Record<string, string> = {
...
const CDM_LEAD_SOURCES = ["Facebook Ads","Google Ads","Direct Mail","Referral","Webinar","Cold Call","TV Ad","Radio Ad","Other"];
const CDM_HEALTH_STATUSES = ["Excellent","Good","Fair","Poor"];
const CDM_BEST_TIMES = ["Morning 8am-12pm","Afternoon 12pm-5pm","Evening 5pm-8pm","Anytime"];
const CDM_LEAD_STATUSES: LeadStatus[] = ["New","Contacted","Interested","Follow Up","Hot","Not Interested","Closed Won","Closed Lost"];

interface CdmConvItem {
  id: string;
  type: "call" | "sms";
...
const CDM_MOCK_THREAD: CdmConvItem[] = [
  { id: "c1", type: "call", direction: "outbound", duration: "3:42", disposition: "Left Voicemail", date: new Date(Date.now() - 6 * 86400000).toISOString() },
  { id: "c2", type: "sms", direction: "inbound", content: "Hi, yes I'm interested! Can you call me back this afternoon?", date: new Date(Date.now() - 5 * 86400000 + 3600000).toISOString() },
  { id: "c3", type: "sms", direction: "outbound", content: "Absolutely! I'll give you a call around 2pm. Looking forward to speaking with you.", date: new Date(Date.now() - 5 * 86400000 + 7200000).toISOString() },
  { id: "c4", type: "call", direction: "inbound", duration: "7:15", disposition: "Interested", date: new Date(Date.now() - 4 * 86400000).toISOString() },
  { id: "c5", type: "sms", direction: "outbound", content: "Great speaking with you today! I'll send over the quote shortly.", date: new Date(Date.now() - 2 * 86400000).toISOString() },
  { id: "c6", type: "call", direction: "outbound", duration: "1:20", disposition: "No Answer", date: new Date(Date.now() - 86400000).toISOString() },
];

function cdmFormatDate(iso: string) {
...
const ContactDetailModal: React.FC<{
  ...
  const inputCls = "w-full h-9 px-3 rounded-md bg-background text-sm text-foreground border border-border focus:ring-2 focus:ring-ring focus:outline-none";

  const filteredThread = CDM_MOCK_THREAD.filter(item => {
    if (convFilter === "All") return true;
    if (convFilter === "Calls") return item.type === "call";
    if (convFilter === "SMS") return item.type === "sms";
    return false;
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={handleClose}>
      ...
      <div
        className="relative bg-card border border-border rounded-2xl shadow-2xl flex flex-col w-full max-w-5xl animate-in fade-in duration-150"
        style={{ height: "85vh" }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-6 py-4 border-b border-border flex items-center justify-between shrink-0 flex-wrap gap-2">
          {/* Left: Avatar + Name + Status */}
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-blue-500 text-white flex items-center justify-center text-lg font-bold shrink-0">
              {lead.firstName[0]}{lead.lastName[0]}
            </div>
            <div>
              <h2 className="text-xl font-bold text-foreground">{lead.firstName} {lead.lastName}</h2>
              <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${cdmStatusBadgeColor[lead.status] || "bg-gray-400 text-white"}`}>{lead.status}</span>
            </div>
          </div>
          {/* Right: Action + Edit buttons */}
          <div className="flex items-center gap-2 flex-wrap">
            <button
              onClick={() => toast.info("Dialer opening...")}
            ...
            <button
              onClick={() => toast.info("Configure SMTP in Settings")}
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg text-white transition-colors"
            ...
            {!editMode ? (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg border border-border bg-background hover:bg-accent text-foreground transition-colors"
              >
              ...
              <>
                <button
                  onClick={handleSave}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground transition-colors"
                >
        ...
        {editMode && hasUnsavedChanges && (
          <div className="mx-6 mb-0 mt-2 flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-lg px-4 py-2 text-sm text-yellow-600 dark:text-yellow-400">
        ...
        <div className="flex flex-1 min-h-0">
          {/* LEFT — Contact Info */}
          <div className="w-[40%] border-r border-border overflow-y-auto p-6 space-y-4">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Contact Info</p>

            {/* Phone with copy */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Phone</label>
              {editMode ? (
                <>
                  <input value={editForm.phone || ""} onChange={e => handleFieldChange("phone", e.target.value)} className={inputCls} />
                  {errors.phone && <p className="text-xs text-red-500 mt-0.5">{errors.phone}</p>}
                </>
              ) : (
                <div className="flex items-center justify-between group mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm min-h-[36px]">
                  <span className="text-foreground font-medium">{lead.phone || "—"}</span>
                  {lead.phone && (
                    <button
                      onClick={() => { navigator.clipboard.writeText(lead.phone); toast.success("Copied to clipboard", { duration: 3000, position: "bottom-right" }); }}
            ...
            <div>
              ...
              {editMode ? (
                <input type="email" value={editForm.email || ""} onChange={e => handleFieldChange("email", e.target.value)} className={inputCls} />
              ) : (
                <div className="flex items-center justify-between group mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm min-h-[36px]">
                  <span className="text-foreground font-medium">{lead.email || "—"}</span>
                  {lead.email && (
                    <button
                      onClick={() => { navigator.clipboard.writeText(lead.email); toast.success("Copied to clipboard", { duration: 3000, position: "bottom-right" }); }}
                      className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-accent text-muted-foreground hover:text-foreground"
                    >
                      <Clipboard className="w-3.5 h-3.5" />
                    </button>
            ...
            {/* First Name */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">First Name</label>
              {editMode ? (
                <>
                  <input value={editForm.firstName || ""} onChange={e => handleFieldChange("firstName", e.target.value)} className={inputCls} />
                  {errors.firstName && <p className="text-xs text-red-500 mt-0.5">{errors.firstName}</p>}
                </>
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.firstName || "—"}</div>
              )}
            </div>

            {/* Last Name */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Last Name</label>
              {editMode ? (
                <>
                  <input value={editForm.lastName || ""} onChange={e => handleFieldChange("lastName", e.target.value)} className={inputCls} />
                  {errors.lastName && <p className="text-xs text-red-500 mt-0.5">{errors.lastName}</p>}
                </>
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.lastName || "—"}</div>
              )}
            </div>

            {/* State */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">State</label>
              {editMode ? (
                <select value={editForm.state || ""} onChange={e => handleFieldChange("state", e.target.value)} className={inputCls}>
                  <option value="">—</option>
                  {CDM_US_STATES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.state || "—"}</div>
              )}
            </div>
            ...
            <div>
              ...
              {editMode ? (
                <select value={editForm.status || ""} onChange={e => handleFieldChange("status", e.target.value)} className={inputCls}>
                  {CDM_LEAD_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.status || "—"}</div>
              )}
            </div>

            {/* Lead Source */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Lead Source</label>
              {editMode ? (
                <select value={editForm.leadSource || ""} onChange={e => handleFieldChange("leadSource", e.target.value)} className={inputCls}>
                  <option value="">—</option>
                  {CDM_LEAD_SOURCES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.leadSource || "—"}</div>
              )}
            </div>

            {/* Lead Score */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Lead Score</label>
              {editMode ? (
                <input type="number" min={1} max={10} value={editForm.leadScore ?? ""} onChange={e => handleFieldChange("leadScore", Number(e.target.value))} className={inputCls} />
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.leadScore ?? "—"}</div>
              )}
            </div>

            {/* Age */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Age</label>
              {editMode ? (
                <input type="number" value={editForm.age ?? ""} onChange={e => handleFieldChange("age", Number(e.target.value))} className={inputCls} />
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.age ?? "—"}</div>
              )}
            </div>

            {/* Date of Birth */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Date of Birth</label>
              {editMode ? (
                <input type="date" value={editForm.dateOfBirth || ""} onChange={e => handleFieldChange("dateOfBirth", e.target.value)} className={inputCls} />
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.dateOfBirth || "—"}</div>
              )}
            </div>

            {/* Health Status */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Health Status</label>
              {editMode ? (
                <select value={editForm.healthStatus || ""} onChange={e => handleFieldChange("healthStatus", e.target.value)} className={inputCls}>
                  <option value="">—</option>
                  {CDM_HEALTH_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.healthStatus || "—"}</div>
              )}
            </div>

            {/* Best Time to Call */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Best Time to Call</label>
              {editMode ? (
                <select value={editForm.bestTimeToCall || ""} onChange={e => handleFieldChange("bestTimeToCall", e.target.value)} className={inputCls}>
                  <option value="">—</option>
                  {CDM_BEST_TIMES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{lead.bestTimeToCall || "—"}</div>
              )}
            </div>

            {/* Notes */}
            <div>
              <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Notes</label>
              {editMode ? (
                <textarea value={editForm.notes || ""} onChange={e => handleFieldChange("notes", e.target.value)} rows={3} className={`${inputCls} min-h-[72px] h-auto py-2`} />
              ) : (
                <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px]">{lead.notes || "—"}</div>
              )}
            </div>
          </div>
            ...
            {activeTab === "Conversations" && (
              ...
              {filteredThread.length === 0 ? (
                ...
                filteredThread.map(item => {
                  if (item.type === "call") {
                    return (
                      <div key={item.id} className="bg-accent/50 rounded-xl px-4 py-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {item.disposition && (
                            <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 font-medium">{item.disposition}</span>
                          )}
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">{cdmFormatDate(item.date)}</p>
                  ...
                  return (
                    <div key={item.id} className={`flex ${isOut ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${isOut ? "bg-primary text-primary-foreground" : "bg-accent text-foreground"}`}>
                        <p className="text-sm">{item.content}</p>
                        <p className={`text-xs mt-1 ${isOut ? "text-primary-foreground/70" : "text-muted-foreground"}`}>{cdmFormatDate(item.date)}</p>
                      </div>
                    </div>
                  );
                })
            ...
            <div className="border-t border-border p-4 shrink-0">
              <div className="flex gap-1 mb-3">
                {(["SMS", "Email"] as const).map(t => (
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-full font-medium transition-colors ${composeTab === t ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground hover:text-foreground"}`}
                  >
                    {t}
                  </button>
                ))}
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 h-9 px-3 rounded-lg bg-background text-sm text-foreground border border-border focus:ring-2 focus:ring-ring focus:outline-none"
                  onKeyDown={e => {
                    if (e.key === "Enter" && !e.shiftKey) {
                      e.preventDefault();
                      if (composeText.trim()) { toast.success("Message sent!", { duration: 3000, position: "bottom-right" }); setComposeText(""); }
                    }
                  }}
                />
                <button
                  onClick={() => toast.success("Templates coming soon", { duration: 3000, position: "bottom-right" })}
                  className="h-9 px-3 rounded-lg border border-border bg-background text-muted-foreground hover:text-foreground hover:bg-accent transition-colors"
                  title="Templates"
                >
                  <FileText className="w-4 h-4" />
                </button>
                <button
                  onClick={() => { if (composeText.trim()) { toast.success("Message sent!", { duration: 3000, position: "bottom-right" }); setComposeText(""); } }}
...
const Contacts: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <ContactDetailModal lead={selectedLead} onClose={() => setSelectedLead(null)} onUpdate={handleUpdateLead} onDelete={handleDeleteLead} onConvert={() => { fetchData(); setSelectedLead(null); }} />

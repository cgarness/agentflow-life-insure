...
import { X, Phone, Mail, Calendar, Pencil, Trash2, Clock, Pin, Headphones, FileText, RefreshCw, MessageSquare, ChevronDown } from "lucide-react";
...
const RecruitModal: React.FC<RecruitModalProps> = ({ recruit, onClose, onUpdate, onDelete }) => {
    ...
    const renderField = (label: string, key: string, type: "text" | "email" | "select" | "textarea" = "text", options?: string[]) => { const val = (editForm as any)[key] ?? ""; return (<div><label className="text-[11px] font-bold text-foreground dark:text-muted-foreground uppercase tracking-wider block mb-1">{label}</label>{editMode ? (type === "select" ? <select value={val} onChange={e => handleFieldChange(key, e.target.value)} className={inp}><option value="">—</option>{options?.map(o => <option key={o} value={o}>{o}</option>)}</select> : type === "textarea" ? <textarea value={val} onChange={e => handleFieldChange(key, e.target.value)} rows={3} className={`${inp} min-h-[72px] py-2`} /> : <input type={type} value={val} onChange={e => handleFieldChange(key, e.target.value)} className={inp} />) : <div className="mt-1 px-3 py-2 rounded-md bg-muted/60 text-sm text-foreground min-h-[36px] flex items-center">{val || "—"}</div>}</div>); }; // eslint-disable-line @typescript-eslint/no-explicit-any

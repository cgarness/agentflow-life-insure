...
const CampaignPerformance: React.FC<Props> = ({ campaigns, loading }) => {
  ...
  const data = campaigns.map(c => ({
    ...
    contacted: c.leads_contacted || 0,
    converted: c.leads_converted || 0,
  ...
  const handleExport = () => {
    downloadCSV("campaign-performance", ["Campaign", "Type", "Total Leads", "Contacted", "Contacted %", "Converted", "Converted %"],
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-right py-2 px-2 text-muted-foreground font-medium">Contacted</th>
                  <th className="text-right py-2 px-2 text-muted-foreground font-medium">Converted</th>
                  <th className="text-right py-2 px-2 text-muted-foreground font-medium">Conv. Rate</th>
                </tr>
              </thead>
              <tbody>
                {data.map(d => (

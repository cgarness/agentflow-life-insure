...
const Sidebar: React.FC = () => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside
        className={`hidden md:flex fixed left-0 top-0 h-screen z-60 sidebar-transition ${collapsed ? "w-16" : "w-60"}`}
      >
        {sidebarContent}
      </aside>
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 h-screen w-60 z-60 md:hidden"
            >
              <button
                onClick={() => setMobileOpen(false)}

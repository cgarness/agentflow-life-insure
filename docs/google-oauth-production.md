...
September 22 recovery: GitHub and Supabase connector reads succeeded again after the earlier HTTP 400 failures. The repository reports write access; Supabase reports `ACTIVE_HEALTHY` and organization plan `pro`. Shell Git authentication is still unavailable, so publish the approved draft through the connected GitHub API and verify that the remote tree is identical to the local reviewed tree. The pre-publication Git read confirmed main at `03bae62870c24a336801a7ff6faee46b8ce69269` with no remote feature branch. All 61 backend and 6 UI checks, root TypeScript and diff checks passed again. Draft publication is not approval to merge or deploy.

The next deletion proposal and outstanding backup evidence are recorded in `docs/google-data-deletion-plan.md`. The local user-deletion path only soft-deletes the profile; it is not proof of stopped Google access or erased history. Live backup retention remains unverified.
